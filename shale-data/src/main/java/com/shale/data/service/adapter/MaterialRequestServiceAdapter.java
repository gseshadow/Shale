...
public final class MaterialRequestServiceAdapter implements MaterialRequestServicePort {
    ...
    public List<MaterialRequestUpdateDto> listUpdates(long caseId,long materialRequestId,int shaleClientId,int actorUserId){ if(readAuditSink!=null) readAuditSink.auditRead("MaterialRequestUpdate.Body", "CASE_MATERIALS_REQUEST_UPDATES", "MaterialRequest", materialRequestId); return dao.listUpdates(caseId,materialRequestId,shaleClientId);}
    public List<MaterialRequestStatusHistoryDto> listStatusHistory(long caseId,List<Long> ids,int shaleClientId){return dao.listStatusHistory(caseId,ids,shaleClientId);}
    public MaterialRequestUpdateDto addNote(AddMaterialRequestNoteCommand command){String body=command.body()==null?null:command.body().trim();if(body==null||body.isEmpty())throw new IllegalArgumentException("Note is required.");if(body.length()>4000)throw new IllegalArgumentException("Note is too long.");return dao.addNote(command);}
