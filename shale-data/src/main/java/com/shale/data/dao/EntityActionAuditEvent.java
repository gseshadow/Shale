...
public record EntityActionAuditEvent(
	...
	public enum EntityType { CONTACT, CONTACT_PHONE_NUMBER, CONTACT_EMAIL_ADDRESS, CONTACT_ADDRESS, ORGANIZATION, ORGANIZATION_PHONE, ORGANIZATION_EMAIL, ORGANIZATION_ADDRESS, ORGANIZATION_WEBSITE, CASE, CASE_OVERVIEW_CONFIGURATION, CASE_DATE_PRESENTATION_CONFIGURATION, CASE_STATUS, LINK_TYPE, CASE_LINK, CASE_LINK_SHARE, CASE_DATE, CASE_DATE_TYPE, CASE_TEAM_ROLE, CASE_TEAM_MEMBER, CASE_TEAM_MEMBER_ROLE, FIRM_WIDE_ROLE, USER_FIRM_WIDE_ROLE, FIELD_CONFIRMATION_POLICY, SAVED_VALUE_CONFIRMATION, CALENDAR_EVENT, CASE_DATE_ROLE_MAPPING, FORM_CONFIGURATION, MATERIAL_TYPE, MATERIAL_REQUEST, MATERIAL_REQUEST_FOLLOW_UP, MATERIAL_ITEM, USER, CONTACT_TYPE, SPECIALTY, CREDENTIAL_DEFINITION, CONTACT_CONTACT_TYPE, CONTACT_SPECIALTY, CONTACT_CREDENTIAL, ORGANIZATION_TYPE, ORGANIZATION_ORGANIZATION_TYPE }
	...
	private static boolean isAllowedCombination(EntityType entityType, Action action) {
		return switch (entityType) {
			...
			case CASE -> action == Action.UPDATED || action == Action.DELETED || action == Action.RESTORED;
			case CASE_OVERVIEW_CONFIGURATION -> action == Action.CREATED || action == Action.UPDATED;
			case CASE_DATE_PRESENTATION_CONFIGURATION -> action == Action.UPDATED;
		...
		case CASE_TEAM_ROLE -> action == Action.CREATED || action == Action.OVERRIDE_CREATED || action == Action.UPDATED || action == Action.ACTIVATED || action == Action.DEACTIVATED || action == Action.REMOVED || action == Action.RESTORED || action == Action.OVERRIDE_RESET;
			case CASE_TEAM_MEMBER -> action == Action.ADDED || action == Action.REMOVED;
			case CASE_TEAM_MEMBER_ROLE -> action == Action.ADDED || action == Action.REMOVED || action == Action.RESTORED;
			case FIRM_WIDE_ROLE -> action == Action.CREATED || action == Action.UPDATED || action == Action.ACTIVATED || action == Action.DEACTIVATED || action == Action.DELETED;
			case USER_FIRM_WIDE_ROLE -> action == Action.ADDED || action == Action.REMOVED || action == Action.RESTORED;
			case FIELD_CONFIRMATION_POLICY -> action == Action.CREATED || action == Action.UPDATED;
			case SAVED_VALUE_CONFIRMATION -> action == Action.CONFIRMED;
			...
			case ORGANIZATION_TYPE -> action == Action.CREATED || action == Action.OVERRIDE_CREATED || action == Action.UPDATED || action == Action.ACTIVATED || action == Action.DEACTIVATED || action == Action.REMOVED || action == Action.RESTORED;
			case ORGANIZATION_ORGANIZATION_TYPE -> action == Action.ADDED || action == Action.REMOVED || action == Action.RESTORED || action == Action.PRIMARY_SET || action == Action.REORDERED;
		};
	}
