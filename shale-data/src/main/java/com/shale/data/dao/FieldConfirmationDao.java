...
public final class FieldConfirmationDao {
    ...
    public FieldConfirmationPolicyDto setPolicy(SetFieldConfirmationPolicyCommand c){
        ...
        try(Connection con=db.requireConnection()){verifySession(con,c.shaleClientId(),c.actorUserId(),true);con.setAutoCommit(false);try{
            ...
            long id;try(PreparedStatement ps=con.prepareStatement("DECLARE @InsertedPolicy TABLE(Id bigint NOT NULL); INSERT dbo.FieldConfirmationPolicies(ShaleClientId,CaseDateTypePolicyKey,PolicyRevision,RequiresConfirmation,RequiredFirmWideRoleDefinitionId,CreatedByUserId) OUTPUT INSERTED.Id INTO @InsertedPolicy(Id) VALUES(?,?,?,?,?,?); SELECT Id FROM @InsertedPolicy;")){ps.setInt(1,c.shaleClientId());ps.setString(2,key);ps.setLong(3,revision);ps.setBoolean(4,c.requiresConfirmation());if(c.requiredFirmWideRoleDefinitionId()==null)ps.setNull(5,Types.INTEGER);else ps.setInt(5,c.requiredFirmWideRoleDefinitionId());ps.setInt(6,c.actorUserId());try(ResultSet rs=ps.executeQuery()){if(!rs.next())throw new IllegalStateException("Policy was not saved.");id=rs.getLong(1);}}
    ...
    public void evaluateCaseDate(Connection con,int tenant,int actor,long caseDateId,int caseDateTypeId,long revision)throws SQLException{
        ...
        long requirement;try(PreparedStatement ps=con.prepareStatement("DECLARE @InsertedRequirement TABLE(Id bigint NOT NULL); INSERT dbo.SavedValueConfirmationRequirements(ShaleClientId,TargetType,BusinessValueRevision,FieldConfirmationPolicyId,PolicyRevisionSnapshot,FormKeySnapshot,FieldKeySnapshot,RequiredFirmWideRoleDefinitionId,EnteredByUserId) OUTPUT INSERTED.Id INTO @InsertedRequirement(Id) VALUES(?,'CASE_DATE',?,?,?,?,?,?,?); SELECT Id FROM @InsertedRequirement;")){ps.setInt(1,tenant);ps.setLong(2,revision);ps.setLong(3,p.id);ps.setLong(4,p.revision);ps.setString(5,"CASE_DATE_TYPE");ps.setString(6,p.policyKey);ps.setInt(7,p.roleId);ps.setInt(8,actor);try(ResultSet rs=ps.executeQuery()){if(!rs.next())throw new IllegalStateException("Confirmation requirement was not saved.");requirement=rs.getLong(1);}}
    ...
    private static boolean hasRole(Connection con,int tenant,int actor,int role)throws SQLException{String sql="""
      ...
      WHERE u.id=? AND u.ShaleClientId=? AND ISNULL(u.is_deleted,0)=0 AND ISNULL(u.IsRemoved,0)=0 AND
       ((r.SystemKey='ADMIN' AND u.is_admin=1) OR (r.SystemKey='ATTORNEY' AND u.is_attorney=1) OR (r.SystemKey NOT IN('ADMIN','ATTORNEY') AND EXISTS(SELECT 1 FROM dbo.UserFirmWideRoleAssignments a WHERE a.ShaleClientId=u.ShaleClientId AND a.UserId=u.id AND a.FirmWideRoleDefinitionId=r.Id AND a.IsDeleted=0)))
