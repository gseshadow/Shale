...
public final class FormConfigurationDao {
    ...
    private static void insertSections(Connection con,long formId,ReplaceCommand c)throws SQLException{
        ...
        String keys="INSERT dbo.FormFieldPolicyKeys(ShaleClientId,FormKey,FieldKey) SELECT ?,?,? WHERE NOT EXISTS(SELECT 1 FROM dbo.FormFieldPolicyKeys WHERE ShaleClientId=? AND FormKey=? AND FieldKey=?)";
        try(PreparedStatement section=con.prepareStatement(ss,Statement.RETURN_GENERATED_KEYS);PreparedStatement field=con.prepareStatement(fs);PreparedStatement key=con.prepareStatement(keys)){
