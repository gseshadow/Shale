...
final class CaseCalendarSynchronizer {
    ...
    void fromCaseDate(Connection con, int tenant, int actor, long caseDateId, String operation) throws SQLException {
        DateRow date = lockDate(con, tenant, caseDateId);
        EventRow linked = lockLinkedEvent(con, tenant, caseDateId);
        Mapping mapping = date.deleted ? null : mappingForDate(con, tenant, date.typeId, true);
        ...
        if (linked == null) {
            ...
            updateEventProjection(con, linked, mapping.eventTypeId, date, actor, false);
            audit(con, tenant, actor, EntityActionAuditEvent.EntityType.CALENDAR_EVENT, linked.id,
                    "RESTORE".equals(operation) ? EntityActionAuditEvent.Action.RESTORED : EntityActionAuditEvent.Action.UPDATED,
                    date.caseId, caseDateId, "CASE_DATE_TO_CALENDAR");
        }
    }

    void fromCalendar(Connection con, int tenant, int actor, int eventId, String operation) throws SQLException {
        ...
        if ("DELETE".equals(operation)) {
            if (linked != null && !linked.deleted) updateDateLifecycle(con, linked, true, actor);
        ...
        if (linked == null) {
            ...
            updateDateProjection(con, linked, mapping.dateTypeId, event, actor);
            audit(con, tenant, actor, EntityActionAuditEvent.EntityType.CASE_DATE, linked.id,
                    event.cancelled ? EntityActionAuditEvent.Action.DELETED : (linked.deleted ? EntityActionAuditEvent.Action.RESTORED : EntityActionAuditEvent.Action.UPDATED),
    ...
    private static void updateEventProjection(Connection c,EventRow e,int type,DateRow d,int a,boolean cancelled)throws SQLException{try(PreparedStatement p=c.prepareStatement("UPDATE dbo.CalendarEvents SET CalendarEventTypeId=?,StartsAt=?,EndsAt=?,AllDay=?,IsCancelled=?,UpdatedAt=SYSUTCDATETIME() WHERE CalendarEventId=? AND RowVer=?")){p.setInt(1,type);p.setTimestamp(2,d.starts);p.setTimestamp(3,d.ends);p.setBoolean(4,d.allDay);p.setBoolean(5,cancelled);p.setInt(6,e.id);p.setBytes(7,e.rowVer);changed(p);}}
    private static void updateDateProjection(Connection c,DateRow d,int type,EventRow e,int a)throws SQLException{try(PreparedStatement p=c.prepareStatement("UPDATE dbo.CaseDates SET CaseDateTypeId=?,StartsAt=?,EndsAt=?,AllDay=?,IsDeleted=?,DeletedAt=CASE WHEN ?=1 THEN SYSUTCDATETIME() ELSE NULL END,DeletedByUserId=CASE WHEN ?=1 THEN ? ELSE NULL END,UpdatedAt=SYSUTCDATETIME(),UpdatedByUserId=? WHERE Id=? AND RowVer=?")){p.setInt(1,type);p.setTimestamp(2,e.starts);p.setTimestamp(3,e.ends);p.setBoolean(4,e.allDay);p.setBoolean(5,e.cancelled);p.setBoolean(6,e.cancelled);p.setBoolean(7,e.cancelled);p.setInt(8,a);p.setInt(9,a);p.setLong(10,d.id);p.setBytes(11,d.rowVer);changed(p);}}
