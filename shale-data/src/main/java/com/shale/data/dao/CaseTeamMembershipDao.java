...
public final class CaseTeamMembershipDao {
	...
	/** Complete-team reconciliation. All validation, compatibility writes, audit and timeline writes share this transaction. */
	public List<CaseTeamMembershipDto> updateTeam(CaseTeamUpdateCommand x){
		Objects.requireNonNull(x,"command");
		Set<Integer> users=new HashSet<>();
		for(var m:x.members()){if(!users.add(m.userId()))throw new IllegalArgumentException("A user may appear only once in the staged team.");if(m.membershipId()<0)throw new IllegalArgumentException("Invalid membership identity.");if(new HashSet<>(m.roleDefinitionIds()).size()!=m.roleDefinitionIds().size())throw new IllegalArgumentException("A role may be assigned only once per member.");}
		tx(x.tenantId(),c->{requireActor(c,x.tenantId(),x.actorUserId());requireCase(c,x.tenantId(),x.caseId());lockResponsible(c,x.tenantId(),x.caseId());
			int responsibleCount=0;for(var m:x.members())for(int role:m.roleDefinitionIds())try(var p=c.prepareStatement("SELECT SystemKey FROM dbo.CaseTeamRoleDefinitions WHERE Id=? AND (ShaleClientId=? OR ShaleClientId IS NULL)")){p.setInt(1,role);p.setInt(2,x.tenantId());try(var r=p.executeQuery()){if(!r.next())throw new IllegalArgumentException("A staged role is inaccessible.");if(RESPONSIBLE_ATTORNEY.equals(r.getString(1)))responsibleCount++;}}if(responsibleCount>1)throw new IllegalArgumentException("A case can have only one Responsible Attorney.");
			Map<Long,byte[]> existing=new LinkedHashMap<>();try(var p=c.prepareStatement("SELECT Id,RowVer FROM dbo.CaseUsers WITH (UPDLOCK,HOLDLOCK) WHERE ShaleClientId=? AND CaseId=?")){p.setInt(1,x.tenantId());p.setLong(2,x.caseId());try(var r=p.executeQuery()){while(r.next())existing.put(r.getLong(1),r.getBytes(2));}}
			Set<Long> retained=new HashSet<>();
			for(var desired:x.members()){
				requireUser(c,x.tenantId(),desired.userId()); long memberId=desired.membershipId();
				if(memberId==0){try(var p=c.prepareStatement("INSERT dbo.CaseUsers(CaseId,UserId,RoleId,IsPrimary,Notes,CreatedAt,UpdatedAt,ShaleClientId) OUTPUT inserted.Id VALUES(?,?,NULL,0,NULL,SYSUTCDATETIME(),SYSUTCDATETIME(),?)")){p.setLong(1,x.caseId());p.setInt(2,desired.userId());p.setInt(3,x.tenantId());try(var r=p.executeQuery()){r.next();memberId=r.getLong(1);}}event(c,x.tenantId(),x.actorUserId(),CASE_TEAM_MEMBER,memberId,ADDED,x.caseId());timeline(c,x.caseId(),x.tenantId(),x.actorUserId(),CaseTimelineWriter.CASE_TEAM_MEMBER_ADDED,"Team member added");}
				else {byte[] current=existing.get(memberId);if(current==null||!Arrays.equals(current,desired.membershipRowVer()))throw new IllegalStateException("Team membership changed. Reload and try again.");try(var p=c.prepareStatement("SELECT 1 FROM dbo.CaseUsers WHERE Id=? AND UserId=?")){p.setLong(1,memberId);p.setInt(2,desired.userId());try(var r=p.executeQuery()){if(!r.next())throw new IllegalStateException("Team membership identity changed.");}}}
				retained.add(memberId); reconcileRoles(c,x,memberId,desired.roleDefinitionIds());
			}
			for(var entry:existing.entrySet())if(!retained.contains(entry.getKey())){long id=entry.getKey();try(var p=c.prepareStatement("DELETE dbo.CaseTeamMemberRoles WHERE ShaleClientId=? AND CaseUserId=?")){p.setInt(1,x.tenantId());p.setLong(2,id);p.executeUpdate();}try(var p=c.prepareStatement("DELETE dbo.CaseUsers WHERE Id=? AND ShaleClientId=? AND RowVer=?")){p.setLong(1,id);p.setInt(2,x.tenantId());p.setBytes(3,entry.getValue());if(p.executeUpdate()!=1)throw new IllegalStateException("Team membership changed. Reload and try again.");}event(c,x.tenantId(),x.actorUserId(),CASE_TEAM_MEMBER,id,REMOVED,x.caseId());timeline(c,x.caseId(),x.tenantId(),x.actorUserId(),CaseTimelineWriter.CASE_TEAM_MEMBER_REMOVED,"Team member removed");}
			return null;});return list(x.tenantId(),x.actorUserId(),x.caseId());
	}

	private void reconcileRoles(Connection c,CaseTeamUpdateCommand x,long memberId,List<Integer> desiredIds)throws SQLException{
		Map<Integer,RoleCurrent> current=new HashMap<>();try(var p=c.prepareStatement("SELECT a.Id,a.CaseTeamRoleDefinitionId,a.IsDeleted,d.SystemKey,d.LegacyRoleId FROM dbo.CaseTeamMemberRoles a JOIN dbo.CaseTeamRoleDefinitions d ON d.Id=a.CaseTeamRoleDefinitionId WHERE a.ShaleClientId=? AND a.CaseUserId=?")){p.setInt(1,x.tenantId());p.setLong(2,memberId);try(var r=p.executeQuery()){while(r.next())current.put(r.getInt(2),new RoleCurrent(r.getLong(1),r.getBoolean(3),r.getString(4),(Integer)r.getObject(5)));}}
		Set<Integer> desired=new LinkedHashSet<>(desiredIds);
		for(var e:current.entrySet())if(!e.getValue().deleted&&!desired.contains(e.getKey())){RoleCurrent old=e.getValue();try(var p=c.prepareStatement("UPDATE dbo.CaseTeamMemberRoles SET IsDeleted=1,DeletedAt=SYSUTCDATETIME(),DeletedByUserId=?,UpdatedAt=SYSUTCDATETIME(),UpdatedByUserId=? WHERE Id=? AND IsDeleted=0")){p.setInt(1,x.actorUserId());p.setInt(2,x.actorUserId());p.setLong(3,old.id);p.executeUpdate();}if(old.legacy!=null)try(var p=c.prepareStatement("UPDATE dbo.CaseUsers SET RoleId=NULL,IsPrimary=0,UpdatedAt=SYSUTCDATETIME() WHERE Id=? AND RoleId=?")){p.setLong(1,memberId);p.setInt(2,old.legacy);p.executeUpdate();}event(c,x.tenantId(),x.actorUserId(),CASE_TEAM_MEMBER_ROLE,old.id,REMOVED,x.caseId());timeline(c,x.caseId(),x.tenantId(),x.actorUserId(),RESPONSIBLE_ATTORNEY.equals(old.key)?CaseTimelineWriter.RESPONSIBLE_ATTORNEY_CHANGED:CaseTimelineWriter.CASE_TEAM_ROLE_REMOVED,RESPONSIBLE_ATTORNEY.equals(old.key)?"Responsible Attorney changed":"Team role removed");}
		for(int roleId:desired){RoleCurrent old=current.get(roleId);if(old!=null&&!old.deleted)continue;Def d=effectiveDefinition(c,x.tenantId(),roleId);long id;if(old!=null){try(var p=c.prepareStatement("UPDATE dbo.CaseTeamMemberRoles SET IsDeleted=0,DeletedAt=NULL,DeletedByUserId=NULL,UpdatedAt=SYSUTCDATETIME(),UpdatedByUserId=? WHERE Id=? AND IsDeleted=1")){p.setInt(1,x.actorUserId());p.setLong(2,old.id);p.executeUpdate();}id=old.id;event(c,x.tenantId(),x.actorUserId(),CASE_TEAM_MEMBER_ROLE,id,RESTORED,x.caseId());}else{try(var p=c.prepareStatement("INSERT dbo.CaseTeamMemberRoles(ShaleClientId,CaseId,CaseUserId,CaseTeamRoleDefinitionId,RoleDefinitionTenantKey,CreatedByUserId) OUTPUT inserted.Id VALUES(?,?,?,?,?,?)")){p.setInt(1,x.tenantId());p.setLong(2,x.caseId());p.setLong(3,memberId);p.setInt(4,roleId);p.setInt(5,d.owner==null?0:d.owner);p.setInt(6,x.actorUserId());try(var r=p.executeQuery()){r.next();id=r.getLong(1);}}event(c,x.tenantId(),x.actorUserId(),CASE_TEAM_MEMBER_ROLE,id,ADDED,x.caseId());}if(d.legacy!=null)syncLegacyOnAssign(c,memberId,d.legacy,RESPONSIBLE_ATTORNEY.equals(d.key));timeline(c,x.caseId(),x.tenantId(),x.actorUserId(),RESPONSIBLE_ATTORNEY.equals(d.key)?CaseTimelineWriter.RESPONSIBLE_ATTORNEY_CHANGED:CaseTimelineWriter.CASE_TEAM_ROLE_ASSIGNED,RESPONSIBLE_ATTORNEY.equals(d.key)?"Responsible Attorney changed":"Team role assigned");}
	}
	private record RoleCurrent(long id,boolean deleted,String key,Integer legacy){}
