...
public final class CaseOverviewConfigurationDao {
    ...
    public CaseOverviewDateConfigurationDto replace(ReplaceCaseOverviewDateConfigurationCommand c) {
        ...
        try(Connection con=db.requireConnection()) {
            ...
            try {
                ...
                validateSelections(con,c.shaleClientId(),c.orderedCaseDateTypeIds());
                ...
                touchCase(con,c.caseId(),c.shaleClientId(),c.actorUserId());
    ...
    public IntakeTakenByMutationResult updateIntakeTakenBy(UpdateIntakeTakenByCommand c) {
        ...
        try(Connection con=db.requireConnection()) {
            ...
            try {
                ...
                try(PreparedStatement ps=con.prepareStatement("UPDATE dbo.Cases SET IntakeTakenByUserId=?,UpdatedAt=SYSUTCDATETIME(),UpdatedByUserId=? WHERE Id=? AND ShaleClientId=? AND ISNULL(IsDeleted,0)=0 AND RowVer=?")){
                    if(target==null)ps.setNull(1,Types.INTEGER);else ps.setInt(1,target.id); ps.setInt(2,c.actorUserId());ps.setLong(3,c.caseId());ps.setInt(4,c.shaleClientId());ps.setBytes(5,c.expectedCaseRowVer());if(ps.executeUpdate()!=1)throw new IllegalStateException("Case changed.");
                }
    ...
    private record Config(long id,byte[] rowVer){} private record CaseIntake(Integer userId,String name,byte[] rowVer){} private record User(int id,String name){}
    ...
    private static CaseIntake readIntake(Connection c,int t,long id)throws SQLException{try(PreparedStatement p=c.prepareStatement("SELECT x.IntakeTakenByUserId,LTRIM(RTRIM(CONCAT(u.name_first,' ',u.name_last))),x.RowVer FROM dbo.Cases x LEFT JOIN dbo.Users u ON u.id=x.IntakeTakenByUserId AND u.ShaleClientId=x.ShaleClientId WHERE x.Id=? AND x.ShaleClientId=? AND ISNULL(x.IsDeleted,0)=0")){p.setLong(1,id);p.setInt(2,t);try(ResultSet r=p.executeQuery()){if(!r.next())throw new IllegalArgumentException("Case is not available for this tenant.");return new CaseIntake((Integer)r.getObject(1),r.getString(2),r.getBytes(3));}}}
    ...
    private static void touchCase(Connection c,long id,int t,int actor)throws SQLException{try(PreparedStatement p=c.prepareStatement("UPDATE dbo.Cases SET UpdatedAt=SYSUTCDATETIME(),UpdatedByUserId=? WHERE Id=? AND ShaleClientId=? AND ISNULL(IsDeleted,0)=0")){p.setInt(1,actor);p.setLong(2,id);p.setInt(3,t);if(p.executeUpdate()!=1)throw new IllegalArgumentException("Case is not available for this tenant.");}}
