...
public final class UserDao {
	...
	public List<FirmWideRoleAssignment> listUserFirmWideRoleAssignmentsForView(int tenant,int actor,int user){
		if(user<=0)throw new IllegalArgumentException("userId must be > 0");
		try(Connection c=db.requireConnection()){requireRoleViewer(c,tenant,actor);requireTenantUser(c,tenant,user);String sql="SELECT a.Id,a.UserId,a.FirmWideRoleDefinitionId,d.Name,a.IsDeleted,a.RowVer FROM dbo.UserFirmWideRoleAssignments a JOIN dbo.FirmWideRoleDefinitions d ON d.Id=a.FirmWideRoleDefinitionId AND d.ShaleClientId=a.ShaleClientId WHERE a.ShaleClientId=? AND a.UserId=? AND d.IsActive=1 AND d.IsDeleted=0 ORDER BY a.IsDeleted,d.SortOrder,d.Name,a.Id";try(PreparedStatement p=c.prepareStatement(sql)){p.setInt(1,tenant);p.setInt(2,user);try(ResultSet r=p.executeQuery()){List<FirmWideRoleAssignment> out=new ArrayList<>();while(r.next())out.add(roleAssignment(r));return List.copyOf(out);}}}catch(SQLException e){throw new RuntimeException("Failed to list user firm-wide roles for user view",e);}
	}
	...
	private static void requireRoleViewer(Connection c,int tenant,int actor)throws SQLException{verifyTenantMatchesSession(c,tenant);Integer sessionActor=requireCurrentPrincipalUserId(c);if(sessionActor!=actor)throw new SecurityException("Requested actor does not match session context.");requireTenantUser(c,tenant,actor);}
