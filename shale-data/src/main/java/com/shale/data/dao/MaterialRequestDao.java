...
public final class MaterialRequestDao {
    ...
    private static void validateCreate(CreateMaterialRequestCommand c){if(blank(c.title())==null)throw new IllegalArgumentException("Title is required.");if(blank(c.title()).length()>255)throw new IllegalArgumentException("Title is too long.");if(c.materialTypeId()<=0)throw new IllegalArgumentException("Material Type is required.");if(blank(c.requestMethod())==null)throw new IllegalArgumentException("Request Method is required.");if(blank(c.status())==null)throw new IllegalArgumentException("Status is required.");if(c.requestedByUserId()<=0)throw new IllegalArgumentException("Requested By is required.");if(c.requestedAt()==null)throw new IllegalArgumentException("Requested At is required.");validateRequestedFrom(c.requestedFromContactId(),c.requestedFromOrganizationId());if(c.expectedResponseDate()!=null&&c.expectedResponseDate().isBefore(c.requestedAt().toLocalDate()))throw new IllegalArgumentException("Due date cannot be before Requested At.");if(c.nextFollowUpAt()!=null&&c.nextFollowUpAt().isBefore(c.requestedAt()))throw new IllegalArgumentException("Next follow-up cannot be before Requested At.");validateInterval(c.followUpIntervalDays());}
