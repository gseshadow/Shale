...
import com.shale.core.service.MaterialRequestServicePort.AddMaterialRequestNoteCommand;
...
public final class MaterialRequestDao {
    ...
    public List<MaterialRequestStatusHistoryDto> listStatusHistory(long caseId,List<Long> requestIds,int tenant){
        if(requestIds==null||requestIds.isEmpty())return List.of();
        List<Long> ids=requestIds.stream().filter(Objects::nonNull).distinct().toList();if(ids.isEmpty())return List.of();
        String marks=String.join(",",Collections.nCopies(ids.size(),"?"));
        String sql=("SELECT mu.Id,mu.ShaleClientId,mu.CaseId,mu.MaterialRequestId,mu.StatusSystemKey,mu.StatusDisplayValue,mu.ActorUserId,"+userDisplayName("u")+" ActorDisplayName,mu.CreatedAt FROM dbo.MaterialRequestUpdates mu JOIN dbo.Users u ON u.Id=mu.ActorUserId AND u.ShaleClientId=mu.ShaleClientId WHERE mu.ShaleClientId=? AND mu.CaseId=? AND mu.MaterialRequestId IN ("+marks+") AND mu.StatusDisplayValue IS NOT NULL ORDER BY mu.CreatedAt,mu.Id");
        try(Connection con=db.requireConnection();PreparedStatement ps=con.prepareStatement(sql)){verifyTenant(con,tenant);int i=1;ps.setInt(i++,tenant);ps.setLong(i++,caseId);for(Long id:ids)ps.setLong(i++,id);try(ResultSet rs=ps.executeQuery()){List<MaterialRequestStatusHistoryDto> out=new ArrayList<>();while(rs.next())out.add(new MaterialRequestStatusHistoryDto(rs.getLong(1),rs.getInt(2),rs.getLong(3),rs.getLong(4),rs.getString(5),rs.getString(6),rs.getInt(7),rs.getString(8),ldt(rs,"CreatedAt")));return out;}}catch(SQLException e){throw sqlFailure(e);}
    }
    ...
    public MaterialRequestDetailDto create(CreateMaterialRequestCommand c){
        ...
        try(Connection con=db.requireConnection()){verifyTenant(con,c.shaleClientId());con.setAutoCommit(false);try{
            ...
            long id=insert(con,c,status.persistedStatus(),closure,next);appendStatusUpdate(con,c.shaleClientId(),c.caseId(),id,"SYSTEM_EVENT","CREATED","Material request created.",status,displayLookup(con,"dbo.RequestStatuses",c.shaleClientId(),status.persistedStatus()),c.actorUserId(),mutationTime);createRecipientNotifications(con,id,c.shaleClientId(),c.caseId(),c.title(),c.actorUserId(),null,null,c.requestedByUserId(),c.assignedToUserId(),"created");
            touchCase(con,c.caseId(),c.shaleClientId());auditPhiCreate(con,c.actorUserId(),id,c);audit(con,c.shaleClientId(),c.actorUserId(),EntityActionAuditEvent.Action.CREATED,id,c.caseId(),meta(c));
            CaseTimelineWriter.append(con,c.caseId(),c.shaleClientId(),c.actorUserId(),CaseTimelineWriter.MATERIAL_REQUEST_CREATED,"added a Material Request","Status: "+displayLookup(con,"dbo.RequestStatuses",c.shaleClientId(),status.persistedStatus()));
            con.commit();return findMaterialRequest(c.caseId(),id,c.shaleClientId());
    ...
    public MaterialRequestDetailDto update(UpdateMaterialRequestCommand c){
        ...
        try(Connection con=db.requireConnection()){verifyTenant(con,c.shaleClientId());con.setAutoCommit(false);try{
            ...
            createRecipientNotifications(con,c.materialRequestId(),c.shaleClientId(),c.caseId(),c.title(),c.actorUserId(),previous.requestedByUserId(),previous.assignedToUserId(),c.requestedByUserId(),c.assignedToUserId(),"update-"+HexFormat.of().formatHex(c.rowVer()));touchCase(con,c.caseId(),c.shaleClientId());audit(con,c.shaleClientId(),c.actorUserId(),EntityActionAuditEvent.Action.UPDATED,c.materialRequestId(),c.caseId(),meta(c));
            boolean statusChanged=!Objects.equals(norm(prior.status()),norm(status.persistedStatus()));
            CaseTimelineWriter.append(con,c.caseId(),c.shaleClientId(),c.actorUserId(),statusChanged?CaseTimelineWriter.MATERIAL_REQUEST_STATUS_CHANGED:CaseTimelineWriter.MATERIAL_REQUEST_UPDATED,statusChanged?"changed Material Request status":"updated a Material Request",statusChanged?"from "+displayLookup(con,"dbo.RequestStatuses",c.shaleClientId(),prior.status())+" to "+displayLookup(con,"dbo.RequestStatuses",c.shaleClientId(),status.persistedStatus()):null);
            con.commit();return findMaterialRequest(c.caseId(),c.materialRequestId(),c.shaleClientId());
        }catch(Exception ex){rollback(con);throw ex;}}catch(SQLException e){throw sqlFailure(e);}}
    ...
    public void softDelete(DeleteMaterialRequestCommand c){
        ...
        try(Connection con=db.requireConnection()){verifyTenant(con,c.shaleClientId());con.setAutoCommit(false);try{
            ...
            touchCase(con,c.caseId(),c.shaleClientId());audit(con,c.shaleClientId(),c.actorUserId(),EntityActionAuditEvent.Action.DELETED,c.materialRequestId(),c.caseId(),null);
            CaseTimelineWriter.append(con,c.caseId(),c.shaleClientId(),c.actorUserId(),CaseTimelineWriter.MATERIAL_REQUEST_REMOVED,"removed a Material Request",null);con.commit();
        }catch(Exception ex){rollback(con);throw ex;}}catch(SQLException e){throw sqlFailure(e);}}
    ...
    public List<MaterialRequestUpdateDto> listUpdates(long caseId,long requestId,int tenant){
        String sql="SELECT mu.Id,mu.ShaleClientId,mu.CaseId,mu.MaterialRequestId,mu.UpdateType,mu.FieldKey,mu.Body,mu.ActorUserId,%s AS ActorDisplayName,mu.CreatedAt FROM dbo.MaterialRequestUpdates mu JOIN dbo.MaterialRequests mr ON mr.Id=mu.MaterialRequestId AND mr.ShaleClientId=mu.ShaleClientId AND mr.CaseId=mu.CaseId JOIN dbo.Users u ON u.Id=mu.ActorUserId AND u.ShaleClientId=mu.ShaleClientId WHERE mu.ShaleClientId=? AND mu.CaseId=? AND mu.MaterialRequestId=? ORDER BY mu.CreatedAt DESC,mu.Id DESC".formatted(userDisplayName("u"));
        try(Connection con=db.requireConnection();PreparedStatement ps=con.prepareStatement(sql)){verifyTenant(con,tenant);ps.setInt(1,tenant);ps.setLong(2,caseId);ps.setLong(3,requestId);try(ResultSet rs=ps.executeQuery()){List<MaterialRequestUpdateDto> out=new ArrayList<>();while(rs.next())out.add(mapUpdate(rs));return out;}}catch(SQLException e){throw sqlFailure(e);}
    }
    ...
    private static MaterialRequestUpdateDto mapUpdate(ResultSet rs)throws SQLException{return new MaterialRequestUpdateDto(rs.getLong("Id"),rs.getInt("ShaleClientId"),rs.getLong("CaseId"),rs.getLong("MaterialRequestId"),rs.getString("UpdateType"),rs.getString("FieldKey"),rs.getString("Body"),rs.getInt("ActorUserId"),blank(rs.getString("ActorDisplayName"))==null?"Unknown user":rs.getString("ActorDisplayName"),ldt(rs,"CreatedAt"));}
    ...
    private long appendStatusUpdate(Connection con,int tenant,long caseId,long requestId,String type,String key,String body,StatusSemantic status,String displayValue,int actor,LocalDateTime at)throws SQLException{try(PreparedStatement ps=con.prepareStatement("INSERT dbo.MaterialRequestUpdates(ShaleClientId,CaseId,MaterialRequestId,UpdateType,FieldKey,Body,ActorUserId,CreatedAt,StatusSystemKey,StatusDisplayValue) OUTPUT INSERTED.Id VALUES(?,?,?,?,?,?,?,?,?,?)")){ps.setInt(1,tenant);ps.setLong(2,caseId);ps.setLong(3,requestId);ps.setString(4,type);ps.setString(5,key);ps.setString(6,body);ps.setInt(7,actor);setTs(ps,8,at);ps.setString(9,status.systemKey());ps.setString(10,displayValue);try(ResultSet rs=ps.executeQuery()){rs.next();return rs.getLong(1);}}}
    ...
    private void appendChangeUpdates(Connection con,MaterialRequestDetailDto p,UpdateMaterialRequestCommand c,StatusSemantic status,ClosureValues closure,LocalDateTime next,LocalDateTime at,boolean derivedSchedule)throws SQLException{
        ...
        String newRequestedFrom=requestedFrom(con,c);
        if(!Objects.equals(requestedFromKey(p),requestedFromKey(c)))change(changes,"REQUESTED_FROM",requestedFrom(p),newRequestedFrom,"Requested From");
        if(!Objects.equals(p.requestedByUserId(),c.requestedByUserId()))change(changes,"REQUESTED_BY",p.requestedByDisplayName(),userName(con,c.shaleClientId(),c.requestedByUserId()),"Requested By");
        if(!Objects.equals(p.assignedToUserId(),c.assignedToUserId()))change(changes,"ASSIGNED_TO",p.assignedToDisplayName(),userName(con,c.shaleClientId(),c.assignedToUserId()),"Assigned To");
        ...
        boolean statusChanged=!Objects.equals(norm(p.status()),norm(status.persistedStatus()));
        if(statusChanged)appendStatusUpdate(con,c.shaleClientId(),c.caseId(),c.materialRequestId(),"SYSTEM_CHANGE","STATUS","Status changed from "+oldStatus+" to "+newStatus+".",status,newStatus,c.actorUserId(),at);
        ...
        String oldKey=statusKey(con,c.shaleClientId(),p.status()),newKey=status.systemKey();if(!isTerminal(oldKey)&&"cancelled".equals(newKey))changes.add(new Change("CANCELLED","Request cancelled."));else if(isTerminal(oldKey)&&!isTerminal(newKey))changes.add(new Change("REOPENED","Request reopened."));else if(!isTerminal(oldKey)&&"closed".equals(newKey))changes.add(new Change("CLOSED","Request closed."));
    ...
    private static String interval(Integer d){return d==null?"None":d==1?"Daily":d==7?"Weekly":d==14?"Every 2 weeks":"Every "+d+" days";}
    private static String requestedFrom(MaterialRequestDetailDto d){return d.requestedFromContactId()!=null?d.requestedFromContactDisplayName():d.requestedFromOrganizationId()!=null?d.requestedFromOrganizationName():d.requestedFromText();}
    private static String requestedFromKey(MaterialRequestDetailDto d){return d.requestedFromContactId()!=null?"CONTACT:"+d.requestedFromContactId():d.requestedFromOrganizationId()!=null?"ORGANIZATION:"+d.requestedFromOrganizationId():blank(d.requestedFromText());}
    private static String requestedFromKey(UpdateMaterialRequestCommand c){return c.requestedFromContactId()!=null?"CONTACT:"+c.requestedFromContactId():c.requestedFromOrganizationId()!=null?"ORGANIZATION:"+c.requestedFromOrganizationId():blank(c.requestedFromText());}
    private String requestedFrom(Connection con,UpdateMaterialRequestCommand c)throws SQLException{return c.requestedFromContactId()!=null?lookupName(con,"dbo.Contacts",c.requestedFromContactId(),c.shaleClientId()):c.requestedFromOrganizationId()!=null?lookupName(con,"dbo.Organizations",c.requestedFromOrganizationId(),c.shaleClientId()):blank(c.requestedFromText());}
    private String userName(Connection con,int tenant,Integer id)throws SQLException{if(id==null)return null;try(PreparedStatement ps=con.prepareStatement("SELECT "+userDisplayName("u")+" FROM dbo.Users u WHERE u.Id=? AND u.ShaleClientId=?")){ps.setInt(1,id);ps.setInt(2,tenant);try(ResultSet rs=ps.executeQuery()){return rs.next()?rs.getString(1):"Unknown user";}}}
    private String lookupName(Connection con,String table,int id,int tenant)throws SQLException{String expression=table.endsWith("Contacts")?contactDisplayName("x"):"x.Name";try(PreparedStatement ps=con.prepareStatement("SELECT "+expression+" FROM "+table+" x WHERE x.Id=? AND (x.ShaleClientId=?"+(table.endsWith("MaterialTypes")?" OR x.ShaleClientId IS NULL":"")+")")){ps.setInt(1,id);ps.setInt(2,tenant);try(ResultSet rs=ps.executeQuery()){return rs.next()?rs.getString(1):"Unknown";}}}
    private String displayLookup(Connection con,String table,int tenant,String value)throws SQLException{if(value==null)return null;try(PreparedStatement ps=con.prepareStatement("SELECT TOP(1) Name FROM "+table+" WHERE (ShaleClientId=? OR ShaleClientId IS NULL) AND IsDeleted=0 AND (LOWER(SystemKey)=LOWER(?) OR LOWER(Name)=LOWER(?)) ORDER BY CASE WHEN ShaleClientId=? THEN 0 ELSE 1 END,Id")){ps.setInt(1,tenant);ps.setString(2,value);ps.setString(3,value);ps.setInt(4,tenant);try(ResultSet rs=ps.executeQuery()){if(rs.next())return rs.getString(1);}}return Arrays.stream(value.replace('_',' ').toLowerCase(Locale.ROOT).split(" ")).map(w->w.isEmpty()?w:Character.toUpperCase(w.charAt(0))+w.substring(1)).collect(java.util.stream.Collectors.joining(" "));}
    private String statusKey(Connection con,int tenant,String value)throws SQLException{StatusSemantic s=resolveStatusSemantic(con,tenant,value);return s.systemKey();}
