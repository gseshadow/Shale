...
public final class MaterialRequestDao {
    ...
    public MaterialRequestDetailDto update(UpdateMaterialRequestCommand c){
        ...
        try(Connection con=db.requireConnection()){verifyTenant(con,c.shaleClientId());con.setAutoCommit(false);try{
            ...
            CaseTimelineWriter.append(con,c.caseId(),c.shaleClientId(),c.actorUserId(),statusChanged?CaseTimelineWriter.MATERIAL_REQUEST_STATUS_CHANGED:CaseTimelineWriter.MATERIAL_REQUEST_UPDATED,statusChanged?"changed Material Request status":"updated a Material Request",statusChanged?"from "+prior.status()+" to "+displayLookup(con,"dbo.RequestStatuses",c.shaleClientId(),status.persistedStatus()):null);
