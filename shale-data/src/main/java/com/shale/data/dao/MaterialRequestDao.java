...
public final class MaterialRequestDao {
    ...
    public record MaterialRequestDueNotificationCandidate(long requestId,int shaleClientId,long caseId,String title,Integer assignedToUserId,int requestedByUserId,LocalDate dueAt,String statusSystemKey){
        public Integer recipientUserId(){return assignedToUserId!=null?assignedToUserId:requestedByUserId>0?requestedByUserId:null;}
    }
    ...
    private static String baseSelect(boolean detail){ return """
        ...
        """.formatted(detail?"mr.Description,":"mr.Description,", userDisplayName("rbu"), userDisplayName("au"), contactDisplayName("ct")); }
    ...
    private int updateRow(Connection con,UpdateMaterialRequestCommand c,String persistedStatus,ClosureValues closure,LocalDateTime normalizedNext)throws SQLException{try(PreparedStatement ps=con.prepareStatement("UPDATE dbo.MaterialRequests SET MaterialTypeId=?,Title=?,Description=?,RequestedFromContactId=?,RequestedFromOrganizationId=?,RequestedFromText=?,RequestMethod=?,Status=?,RequestedByUserId=?,AssignedToUserId=?,RequestedAt=?,RelevantStartDate=?,RelevantEndDate=?,ExpectedResponseDate=?,NextFollowUpAt=?,FollowUpIntervalDays=?,FirstReceivedAt=?,FullyReceivedAt=?,ClosedAt=?,ClosedByUserId=?,ClosureReason=?,Notes=?,UpdatedByUserId=?,UpdatedAt=SYSUTCDATETIME() WHERE ShaleClientId=? AND CaseId=? AND Id=? AND IsDeleted=0 AND RowVer=?")){int i=1;ps.setInt(i++,c.materialTypeId());ps.setString(i++,blank(c.title()));ps.setString(i++,blank(c.description()));setInt(ps,i++,c.requestedFromContactId());setInt(ps,i++,c.requestedFromOrganizationId());ps.setString(i++,blank(c.requestedFromText()));ps.setString(i++,norm(c.requestMethod()));ps.setString(i++,persistedStatus);ps.setInt(i++,c.requestedByUserId());setInt(ps,i++,c.assignedToUserId());setTs(ps,i++,c.requestedAt());setDate(ps,i++,c.relevantStartDate());setDate(ps,i++,c.relevantEndDate());setDate(ps,i++,c.expectedResponseDate());setTs(ps,i++,normalizedNext);setInt(ps,i++,c.followUpIntervalDays());setTs(ps,i++,c.firstReceivedAt());setTs(ps,i++,c.fullyReceivedAt());setTs(ps,i++,closure.closedAt());setInt(ps,i++,closure.closedByUserId());ps.setString(i++,closure.closureReason());ps.setString(i++,blank(c.notes()));ps.setInt(i++,c.actorUserId());ps.setInt(i++,c.shaleClientId());ps.setLong(i++,c.caseId());ps.setLong(i++,c.materialRequestId());ps.setBytes(i++,c.rowVer());return ps.executeUpdate();}}
    record RecipientSnapshot(int requestedByUserId,Integer assignedToUserId) {}
    private RecipientSnapshot findRecipientSnapshot(Connection con,UpdateMaterialRequestCommand c)throws SQLException{try(PreparedStatement ps=con.prepareStatement("SELECT RequestedByUserId,AssignedToUserId FROM dbo.MaterialRequests WHERE ShaleClientId=? AND CaseId=? AND Id=? AND IsDeleted=0")){ps.setInt(1,c.shaleClientId());ps.setLong(2,c.caseId());ps.setLong(3,c.materialRequestId());try(ResultSet rs=ps.executeQuery()){if(!rs.next())throw new IllegalArgumentException("Material request not found.");return new RecipientSnapshot(rs.getInt(1),(Integer)rs.getObject(2));}}}
    private void createRecipientNotifications(Connection con,long requestId,int tenant,long caseId,String requestTitle,int actor,Integer oldRequester,Integer oldAssignee,int newRequester,Integer newAssignee,String occurrence)throws SQLException{
        boolean requesterChanged=oldRequester==null||oldRequester!=newRequester;
        boolean assigneeChanged=newAssignee!=null&&!Objects.equals(oldAssignee,newAssignee);
        LinkedHashSet<Integer> recipients=new LinkedHashSet<>();if(requesterChanged&&newRequester!=actor)recipients.add(newRequester);if(assigneeChanged&&newAssignee!=actor)recipients.add(newAssignee);
        for(int recipient:recipients){boolean requester=requesterChanged&&recipient==newRequester,assignee=assigneeChanged&&Objects.equals(recipient,newAssignee);String message=requester&&assignee?"You were assigned to and designated as the requester for a material request.":assignee?"A material request was assigned to you.":"You were designated as the requester for a material request.";String key="material-request:"+requestId+":recipient:"+occurrence+":"+recipient;notifications.createMaterialRequestNotification(con,tenant,recipient,blank(requestTitle),message,requestId,actor,requester&&assignee?"ASSIGNED_AND_REQUESTER":assignee?"ASSIGNED":"REQUESTER",key);}
    }
    ...
    private static void validateCreate(CreateMaterialRequestCommand c){if(blank(c.title())==null)throw new IllegalArgumentException("Title is required.");if(blank(c.title()).length()>255)throw new IllegalArgumentException("Title is too long.");if(c.materialTypeId()<=0)throw new IllegalArgumentException("Material Type is required.");if(blank(c.requestMethod())==null)throw new IllegalArgumentException("Request Method is required.");if(blank(c.status())==null)throw new IllegalArgumentException("Status is required.");if(c.requestedByUserId()<=0)throw new IllegalArgumentException("Requested By is required.");if(c.requestedAt()==null)throw new IllegalArgumentException("Requested At is required.");if(c.requestedFromContactId()!=null&&c.requestedFromOrganizationId()!=null)throw new IllegalArgumentException("Choose either a contact or an organization, not both.");if(c.expectedResponseDate()!=null&&c.expectedResponseDate().isBefore(c.requestedAt().toLocalDate()))throw new IllegalArgumentException("Due date cannot be before Requested At.");if(c.nextFollowUpAt()!=null&&c.nextFollowUpAt().isBefore(c.requestedAt()))throw new IllegalArgumentException("Next follow-up cannot be before Requested At.");validateInterval(c.followUpIntervalDays());}
