...
public final class MaterialRequestDao {
    ...
    private void appendChangeUpdates(Connection con,MaterialRequestDetailDto p,UpdateMaterialRequestCommand c,StatusSemantic status,ClosureValues closure,LocalDateTime next,LocalDateTime at)throws SQLException{
        ...
        change(changes,"REQUESTED_FROM",requestedFrom(p),requestedFrom(con,c),"Requested From");
        change(changes,"REQUESTED_BY",p.requestedByDisplayName(),userName(con,c.shaleClientId(),c.requestedByUserId()),"Requested By");
        change(changes,"ASSIGNED_TO",p.assignedToDisplayName(),userName(con,c.shaleClientId(),c.assignedToUserId()),"Assigned To");
        ...
        String oldKey=statusKey(con,c.shaleClientId(),p.status()),newKey=status.systemKey();if(!Set.of("closed","cancelled").contains(oldKey)&&"cancelled".equals(newKey))changes.add(new Change("CANCELLED","Request cancelled."));else if(Set.of("closed","cancelled").contains(oldKey)&&!Set.of("closed","cancelled").contains(newKey))changes.add(new Change("REOPENED","Request reopened."));else if(!Set.of("closed","cancelled").contains(oldKey)&&"closed".equals(newKey))changes.add(new Change("CLOSED","Request closed."));
    ...
    private static String friendly(Object v){if(v instanceof LocalDate d)return d.format(java.time.format.DateTimeFormatter.ofPattern("MMMM d, yyyy",Locale.US));if(v instanceof LocalDateTime d)return d.format(java.time.format.DateTimeFormatter.ofPattern("MMMM d, yyyy 'at' h:mm a",Locale.US));return String.valueOf(v);}
