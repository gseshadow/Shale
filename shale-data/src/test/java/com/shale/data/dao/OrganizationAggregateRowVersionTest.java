...
final class OrganizationAggregateRowVersionTest {
    ...
    private static final class UpdateDb implements com.shale.core.runtime.DbSessionProvider {
        ...
        byte[] organizationRowVer=rv(3),submittedOrganizationRowVer=rv(3);int compatibilityType=12;String name="Org";long nextId=3;int nextVersion=4;
        int organizationUpdates,openingTokenPredicates,committedAudits;boolean failPrimaryAudit;List<String> pendingAudits=new ArrayList<>();Snapshot snapshot;
        UpdateDb(){assignments.put(1L,new A(1,12,true,0,false,rv(1)));assignments.put(2L,new A(2,13,false,1,false,rv(2)));}
        ...
        public Connection requireConnection(){return (Connection)Proxy.newProxyInstance(Connection.class.getClassLoader(),new Class[]{Connection.class},(p,m,a)->switch(m.getName()){
            ...
            case "setAutoCommit"->{if(!(boolean)a[0]){events.add("begin");snapshot=new Snapshot(name,compatibilityType,organizationRowVer.clone(),copyAssignments());}else events.add("restore-auto");yield null;}
        ...
        private PreparedStatement statement(String sql){Map<Integer,Object>b=new HashMap<>();return(PreparedStatement)Proxy.newProxyInstance(PreparedStatement.class.getClassLoader(),new Class[]{PreparedStatement.class},(p,m,a)->switch(m.getName()){
            case "setInt","setLong","setString","setBoolean","setBytes","setNull"->{b.put((Integer)a[0],a.length>1?a[1]:null);yield null;}
        ...
        private void restore(){if(snapshot==null)return;name=snapshot.name;compatibilityType=snapshot.type;organizationRowVer=snapshot.rowVer;assignments.clear();snapshot.assignments.forEach((id,a)->assignments.put(id,a.copy()));}
        private record Snapshot(String name,int type,byte[]rowVer,Map<Long,A>assignments){}
    }
