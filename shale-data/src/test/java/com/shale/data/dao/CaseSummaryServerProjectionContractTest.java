...
class CaseSummaryServerProjectionContractTest {
    ...
    @Test void serverProjectionIsBoundedTenantScopedAndAuthoritative() throws Exception {
        ...
        assertAll(
            ...
            ()->assertTrue(body.contains("m.SemanticRoleKey='INTAKE'")),
            ()->assertTrue(body.contains("family_type.SystemKey)))='statute_of_limitations'")),
            ()->assertTrue(body.contains("family_type.SystemKey)))='tort_notice_deadline'")),
            ()->assertTrue(body.contains("ORDER BY family_date.StartsAt ASC,family_date.Id ASC")),
            ()->assertFalse(body.contains("SemanticRoleKey='STATUTE_OF_LIMITATIONS'")),
            ()->assertFalse(body.contains("SemanticRoleKey='TORT_NOTICE_DEADLINE'")),
