...
class CaseTeamRoleDefinitionContractTest {
 ...
 @Test void effectiveReadMasksInactiveAndFallsBackOnlyAfterDeletedOverride(){String read=DAO.substring(DAO.indexOf("public List<CaseTeamRoleDefinitionDto> listForAdministration"),DAO.indexOf("public CaseTeamRoleDefinitionDto create"));assertTrue(read.contains("CASE WHEN d.ShaleClientId=? THEN 0 ELSE 1 END"));assertTrue(read.contains("d.ShaleClientId IS NULL OR d.IsDeleted=0"));assertFalse(read.contains("IsActive=1"));assertTrue(read.contains("d.SystemKey IS NULL"));}
