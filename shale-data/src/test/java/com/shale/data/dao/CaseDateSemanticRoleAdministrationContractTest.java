...
final class CaseDateSemanticRoleAdministrationContractTest {
 ...
 @Test void lifecycleIsProtectedAndHistoryIsNotRewritten()throws Exception{String s=source("src/main/java/com/shale/data/dao/CaseDateDao.java");assertTrue(s.contains("requireNotActivelyMapped"));assertTrue(s.contains("SemanticRoleKey='INTAKE'"));assertTrue(s.contains("Only Intake is an operative protected Case Date role."));assertTrue(s.contains("Change or reset the "));assertFalse(s.contains("UPDATE dbo.CaseDates SET CaseDateTypeId")&&s.contains("SemanticRoleMappings SET CaseDateTypeId"));}
