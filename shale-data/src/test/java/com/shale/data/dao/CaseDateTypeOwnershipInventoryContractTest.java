...
final class CaseDateTypeOwnershipInventoryContractTest {
	...
	@Test
	void inventoryIsReadOnlyAndCoversProductionOwnershipEvidence() throws Exception {
	...
	@Test
	void inventoryNamesEveryNoncriticalCompatibilityKey() throws Exception {
		String sql = Files.readString(INVENTORY);
		for (String key : new String[] { "trial", "hearing", "mediation", "deposition", "discovery_deadline",
			...
			assertTrue(sql.contains("N'" + key + "'"), key);
	}

	@Test
	void ownershipCorrectionIsTransactionalIdempotentAndIdentityPreserving() throws Exception {
	...
	@Test
	void ownershipCorrectionFreezesAuthoritativeProductionIdentity() throws Exception {
		String sql = Files.readString(MIGRATION);
		for (String identity : new String[] { "(7,N'trial',N'Trial')", "(8,N'hearing',N'Hearing')",
				...
				"(13,N'non_engagement_letter_sent',N'Non-Engagement Letter Sent')" })
			assertTrue(sql.contains(identity), identity);
		...
		for (String blocker : new String[] { "missing or ambiguous", "identity or lifecycle", "conflicting ownership",
				...
				"unreviewed Case Date Type foreign-key consumer", "personal test tenant" })
			assertTrue(sql.contains(blocker), blocker);
	}
}
