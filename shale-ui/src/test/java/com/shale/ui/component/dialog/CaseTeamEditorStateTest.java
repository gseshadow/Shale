...
class CaseTeamEditorStateTest {
    ...
    @Test void eligibleUsersAppearSortedBeforeTypingAndFilterByNameOrInitials(){var zoe=user(3,"Zoe Young");var amy=user(1,"Amy Jaramillo");var brian=user(2,"Brian Downing");var s=new CaseTeamEditorState(List.of(zoe,brian,amy),List.of(member(7,amy)),List.of());assertEquals(List.of(brian,zoe),s.search(""),"empty search must browse all eligible users in display-name order");assertEquals(List.of(brian),s.search("down"),"typing must filter the visible candidates by display name");assertEquals(List.of(brian),s.search("bd"),"initials must identify the same stable user candidate");assertTrue(s.search("missing").isEmpty(),"unmatched filters must support the visible filtered-empty state");}
