...
class CaseTeamEditorStateTest {
    ...
    @Test void searchSelectionStagesRolelessMemberAndExcludesItUntilRemoved(){var amy=user(1,"Amy Jaramillo");var brian=user(2,"Brian Downing");var s=new CaseTeamEditorState(List.of(amy,brian),List.of(member(7,amy)),List.of());assertEquals(List.of(brian),s.search("Brian"));var staged=s.addMember(brian);assertTrue(staged.roleIds().isEmpty(),"new membership must be roleless");assertTrue(s.search("Brian").isEmpty(),"staged members cannot be offered twice");s.removeMember(brian.id());assertEquals(List.of(brian),s.search("Brian"),"removed staged member must be searchable again");}
