...
class UserDictionarySessionTest {
    ...
    @Test void failedRemovalNeverEvictsAnAcceptedWordFromTheSpellcheckingCache() {
        ...
        session.add("persistedterm");port.failRemove=true;
        assertThrows(RuntimeException.class,()->session.remove("persistedterm"));
        assertFalse(session.checker().isMisspelled("persistedterm"),"cache invalidation must follow, never precede, successful persistence");
    }
    ...
    private static class MemoryPort implements UserDictionaryServicePort {
        private final Map<String,UserDictionaryWord> rows=new HashMap<>();private long id;private boolean failRemove;
        ...
        public void removeWord(int t,int u,String w,int a){if(failRemove)throw new RuntimeException("offline");rows.remove(key(t,u,w));}
    }
}
