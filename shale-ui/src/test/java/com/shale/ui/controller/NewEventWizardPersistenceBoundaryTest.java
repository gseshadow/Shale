...
class NewEventWizardPersistenceBoundaryTest {
    @Test void controllerRoutesEachStableBranchOnlyToItsAuthoritativeService() throws Exception {
    ...
    @Test void wizardGuardsAsyncResultsAndDuplicateSubmissionAndUsesMiniCards() throws Exception {
        String source=Files.readString(Path.of("src/main/java/com/shale/ui/component/dialog/NewEventWizard.java"));
        assertTrue(source.contains("generation==requestGeneration&&step==expected"));
        assertTrue(source.contains("submitting.compareAndSet(false,true)"));
