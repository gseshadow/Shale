...
public final class TeamEditorDialog {
    ...
    private final Label searchEmpty = new Label("No eligible users match this search.");
    ...
    public TeamEditorDialog(Stage owner, CaseServicePort service, int tenantId, int actorId, long caseId,
        ...
        results.setMaxHeight(150);results.getStyleClass().add("case-team-search-results");results.setPlaceholder(searchEmpty);
        results.setCellFactory(v->new ListCell<>(){protected void updateItem(CaseDao.UserRow u,boolean empty){super.updateItem(u,empty);setText(empty||u==null?null:u.displayName());setAccessibleText(empty||u==null?null:"Add "+u.displayName()+" to the case team");}});
    ...
    private void addSelectedResult(){CaseDao.UserRow u=results.getSelectionModel().getSelectedItem();if(u==null)return;state.addMember(u);search.clear();results.getItems().clear();results.setVisible(false);results.setManaged(false);refreshMembers();}
    private void refreshResults(){String q=search.getText();boolean show=q!=null&&!q.isBlank();results.setVisible(show);results.setManaged(show);if(show)results.setItems(FXCollections.observableArrayList(state.search(q)));}
