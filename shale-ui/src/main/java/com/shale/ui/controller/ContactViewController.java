...
public final class ContactViewController {
    ...
    private Window editorOwner;
    private Consumer<Integer> onContactCreated;
    ...
    private void applyResponsiveLayout(double width) {
        ...
        if(notesSection!=null)GridPane.setRowIndex(notesSection,narrow?3:2);
    ...
    public void showCreateEditor(Window owner, AppState state, ContactServicePort service,
            List<ContactServicePort.Definition> types, List<ContactServicePort.Definition> specialties,
            List<ContactServicePort.CredentialDefinition> credentials, Consumer<Integer> created) {
        createMode=true;editorOwner=owner;appState=state;contactService=service;effectiveTypes=List.copyOf(types);
        effectiveSpecialties=List.copyOf(specialties);effectiveCredentials=List.copyOf(credentials);onContactCreated=created;
        int tenant=state.getShaleClientId();classificationProfile=new ContactServicePort.ClassificationProfile(0,tenant,
                new ContactServicePort.StructuredName(null,null,null,null,null,null),null,null,null,null,false,null,
    ...
    private void showProfileEditor(){
        ...
        Runnable composeBase=()->display.setText(com.shale.core.service.ContactNamePresentation.baseDisplayName(
                new ContactServicePort.StructuredName(prefix.getText(),first.getText(),middle.getText(),last.getText(),preferred.getText(),suffix.getText())));
        ...
        saveButton.addEventFilter(javafx.event.ActionEvent.ACTION,e->{
            ...
            try{
                if(notes.getText()!=null&&notes.getText().length()>ContactServicePort.CONTACT_NOTES_MAX_CHARS)throw new IllegalArgumentException("Notes exceeds the supported database length.");
                if(birth.getValue()!=null&&birth.getValue().isAfter(LocalDate.now()))throw new IllegalArgumentException("Date of Birth cannot be in the future.");
                if(safe(display.getText()).isBlank()&&safe(first.getText()).isBlank()&&safe(last.getText()).isBlank())throw new IllegalArgumentException("Display Name, First Name, or Last Name is required.");
                phones.validate();emails.validate();addresses.validate();saveInFlight=true;saveButton.setDisable(true);status.setText(createMode?"Creating Contact…":"Saving complete Contact…");status.setVisible(true);status.setManaged(true);
                var name=new ContactServicePort.StructuredName(prefix.getText(),first.getText(),middle.getText(),last.getText(),preferred.getText(),suffix.getText());
                dbExec.submit(()->{try{
                    ContactServicePort.ContactProfileMutationResult result;
                    if(createMode)result=contactService.createContactProfile(new ContactServicePort.CreateContactProfileCommand(appState.getShaleClientId(),appState.getUserId(),display.getText(),name,birth.getValue(),condition.getText(),notes.getText(),deceased.isSelected(),types.intent(),specs.intent(),creds.intent(),phones.intent(),emails.intent(),addresses.intent()));
                    else result=contactService.updateContactProfile(new ContactServicePort.UpdateContactProfileCommand(contactId,currentContact.shaleClientId(),appState.getUserId(),display.getText(),name,birth.getValue(),condition.getText(),notes.getText(),deceased.isSelected(),p.contactUpdatedAt(),types.intent(),specs.intent(),creds.intent(),phones.intent(),emails.intent(),addresses.intent()));
                    Platform.runLater(()->{if(disposed)return;saveInFlight=false;dialog.close();if(createMode){if(onContactCreated!=null)onContactCreated.accept(result.profile().contactId());}else{classificationProfile=result.profile();renderClassifications();renderContactPoints();contactDetailService.invalidateContact(contactId,currentContact.shaleClientId());if(runtimeBridge!=null)runtimeBridge.publishEntityUpdated(LiveUpdateEvents.ENTITY_CONTACT,contactId,currentContact.shaleClientId(),appState.getUserId(),null);loadContact();}});
