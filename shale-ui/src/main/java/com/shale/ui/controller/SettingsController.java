...
public final class SettingsController {
	...
	@FXML
	private HBox caseDateTypeActionRow;
	@FXML
	...
	@FXML
	private HBox materialTypeActionRow;
	@FXML
	private Label materialTypeSettingsStatusLabel;
	@FXML
	private VBox requestMethodCardsContainer;
	@FXML
	private HBox requestMethodActionRow;
	@FXML
	private Label requestMethodSettingsStatusLabel;
	@FXML
	private VBox requestStatusCardsContainer;
	@FXML
	private HBox requestStatusActionRow;
	@FXML
	...
	private void loadRequestLookupsAsync() {
		if (materialRequestService == null)
			return;
		...
		try {
			...
			setMaterialTypeMessage(rootMessage(ex));
			return;
		}
		...
		settingsLoadExecutor.submit(() ->
		{
			try {
				...
				Platform.runLater(() ->
				{
					if (generation != requestLookupLoadGeneration)
						return;
				...
				Platform.runLater(() ->
				{
					if (generation != requestLookupLoadGeneration)
						return;
					if (materialTypeCardsContainer != null)
						materialTypeCardsContainer.getChildren().setAll(loadingLabel("Failed to load material types. " + rootMessage(ex)));
					if (requestMethodCardsContainer != null)
						requestMethodCardsContainer.getChildren().setAll(loadingLabel("Failed to load request methods. " + rootMessage(ex)));
					if (requestStatusCardsContainer != null)
						requestStatusCardsContainer.getChildren().setAll(loadingLabel("Failed to load request statuses. " + rootMessage(ex)));
				});
			}
		});
	}
	...
	private void showRequestLookupLoadingIfEmpty(VBox container, String message) {
		if (container != null && container.getChildren().isEmpty())
			container.getChildren().setAll(loadingLabel(message));
	}

	private void setCaseStatusLoadingState(String message) {
		if (caseStatusCardsContainer != null)
			caseStatusCardsContainer.getChildren().setAll(loadingLabel(message));
	...
	private void applyCaseStatusRows(int generation, List<CaseStatusViewRow> rows, String successMessage) {
		if (generation != caseStatusLoadGeneration)
			return;
	...
	private void configureLookupActionRows() {
		...
		if (materialTypeActionRow != null)
			configureRequestActionRow(materialTypeActionRow, "Material Type", materialTypeSettingsStatusLabel, this::onAddMaterialType, this::onEditMaterialType,
					this::onToggleMaterialTypeActive, this::onResetOrRemoveMaterialType);
		if (requestMethodActionRow != null)
			configureRequestActionRow(requestMethodActionRow, "Request Method", requestMethodSettingsStatusLabel, this::onAddRequestMethod, this::onEditRequestMethod,
					this::onToggleRequestMethodActive, this::onResetOrRemoveRequestMethod);
		if (requestStatusActionRow != null)
			configureRequestActionRow(requestStatusActionRow, "Request Status", requestStatusSettingsStatusLabel, this::onAddRequestStatus, this::onEditRequestStatus,
					this::onToggleRequestStatusActive, this::onResetOrRemoveRequestStatus);
	}
	...
	private void renderMaterialTypeCards(List<MaterialTypeDto> rows) {
		if (materialTypeCardsContainer == null)
			return;
		List<MaterialTypeDto> effective = rows == null ? List.of() : rows;
		materialTypeCardsContainer.getChildren().setAll(effective.isEmpty() ? List.of(loadingLabel("No material types are configured for this tenant."))
				: effective.stream().map(r -> buildRequestLookupCard(RequestLookupKind.MATERIAL_TYPE, RequestLookupSelection.material(r))).toList());
		setMaterialTypeMessage("");
	}

	private void renderRequestMethodCards(List<RequestMethodDto> rows) {
		if (requestMethodCardsContainer == null)
			return;
		List<RequestMethodDto> effective = rows == null ? List.of() : rows;
		requestMethodCardsContainer.getChildren().setAll(effective.isEmpty() ? List.of(loadingLabel("No request methods are configured for this tenant."))
				: effective.stream().map(r -> buildRequestLookupCard(RequestLookupKind.REQUEST_METHOD, RequestLookupSelection.method(r))).toList());
		setRequestMethodMessage("");
	}

	private void renderRequestStatusCards(List<RequestStatusDto> rows) {
		if (requestStatusCardsContainer == null)
			return;
		List<RequestStatusDto> effective = rows == null ? List.of() : rows;
		requestStatusCardsContainer.getChildren().setAll(effective.isEmpty() ? List.of(loadingLabel("No request statuses are configured for this tenant."))
				: effective.stream().map(r -> buildRequestLookupCard(RequestLookupKind.REQUEST_STATUS, RequestLookupSelection.status(r))).toList());
	...
	private VBox buildRequestLookupCard(RequestLookupKind kind, RequestLookupSelection row) {
		...
		String css = safe(ColorUtil.toCssBackgroundColorOrNull(row.color()));
		if (!css.isBlank())
			dot.setStyle("-fx-background-color: " + css + "; -fx-fill: " + css + ";");
		Label name = new Label(row.name());
		name.getStyleClass().add("app-dialog-field-label");
		Region spacer = new Region();
		HBox.setHgrow(spacer, Priority.ALWAYS);
		header.getChildren().addAll(dot, name, spacer, LinkTypeIndicatorFactory.createLinkTypePill(row.name(), row.color(), LinkTypeIndicatorFactory.PillSize.COMPACT));
		HBox metadata = new HBox(6);
		metadata.setAlignment(Pos.CENTER_LEFT);
		metadata.getChildren().addAll(metadataPill(row.active() ? "Active" : "Inactive"), metadataPill(row.scopeLabel()));
		if (!row.systemKey().isBlank())
			metadata.getChildren().add(metadataPill("System: " + row.systemKey()));
		if (!row.color().isBlank())
			metadata.getChildren().add(metadataPill(row.color()));
		...
		{
			selectRequestLookup(kind, row);
			resetRequestLookup(kind);
			e.consume();
		});
		Label help = new Label(row.description().isBlank() ? row.lifecycleText() : row.description());
	...
	@FXML
	private void onAddCaseDateType() {
		if (!requireAdminLookupManagement("Case Date Types"))
			return;
		showCaseDateTypeDialog(null).ifPresent(input ->
		{
			try {
				EffectiveCaseDateTypeDto saved = caseService.createCaseDateType(new CaseServicePort.CaseDateTypeCommand(null, requireTenantId(), requireActorUserId(), input
						.systemKey(), input.name(), input.description(), input.category(), input.color(), input.supportsTime(), input.sortOrder(), input.active(), null));
				publishCaseDateTypeChanged(saved.id());
				loadCaseDateTypesAsync("Case date type added.");
			} catch (RuntimeException ex) {
	...
	@FXML
	private void onEditCaseDateType() {
		if (!requireAdminLookupManagement("Case Date Types"))
			return;
		CaseDateTypeViewRow selected = selectedCaseDateTypeRow();
		if (selected == null)
			return;
		if (!selected.canEdit()) {
			explainProtectedCaseDateType();
			return;
		}
		showCaseDateTypeDialog(selected.type()).ifPresent(input ->
		{
			try {
				caseService.updateCaseDateType(new CaseServicePort.CaseDateTypeCommand(selected.id(), requireTenantId(), requireActorUserId(), selected.systemKeyForSave(), input
						.name(), input.description(), input.category(), input.color(), input.supportsTime(), input.sortOrder(), input.active(), selected.rowVer()));
				publishCaseDateTypeChanged(selected.id());
				loadCaseDateTypesAsync("Case date type updated.");
			} catch (RuntimeException ex) {
				showCaseDateTypeError(ex);
			}
		});
	...
	@FXML
	private void onToggleCaseDateTypeActive() {
		if (!requireAdminLookupManagement("Case Date Types"))
			return;
		CaseDateTypeViewRow selected = selectedCaseDateTypeRow();
		if (selected == null)
			return;
		if (!selected.canToggleActive()) {
			explainProtectedCaseDateType();
			return;
		}
		try {
			caseService.setCaseDateTypeActive(new CaseServicePort.SetCaseDateTypeActiveCommand(requireTenantId(), requireActorUserId(), selected.id(), !selected.active(), selected
					.rowVer()));
			publishCaseDateTypeChanged(selected.id());
			loadCaseDateTypesAsync(selected.active() ? "Case date type deactivated for new occurrences." : "Case date type activated.");
		} catch (RuntimeException ex) {
			showCaseDateTypeError(ex);
		}
	}
	...
	@FXML
	private void onResetOrRemoveCaseDateType() {
		if (!requireAdminLookupManagement("Case Date Types"))
			return;
		CaseDateTypeViewRow selected = selectedCaseDateTypeRow();
		if (selected == null)
		...
		if (!AppDialogs.showConfirmation(caseDateTypeCardsContainer == null || caseDateTypeCardsContainer.getScene() == null ? null
				: caseDateTypeCardsContainer.getScene().getWindow(), "Case Date Types", action + " " + selected.name() + "?",
				"This affects future type selections only. Existing Case Dates retain their stored type presentation.", action, AppDialogs.DialogActionKind.DANGER))
			return;
		try {
			caseService.resetCaseDateTypeOverride(new CaseServicePort.ResetCaseDateTypeOverrideCommand(requireTenantId(), requireActorUserId(), selected.id(), selected.rowVer()));
			publishCaseDateTypeChanged(selected.id());
			loadCaseDateTypesAsync("Custom case date type removed from future selections.");
		} catch (RuntimeException ex) {
			showCaseDateTypeError(ex);
		}
	}

	private void loadCaseDateTypesAsync(String successMessage) {
		if (caseService == null || caseDateTypeCardsContainer == null)
			return;
		if (!requireAdminLookupManagement("Case Date Types")) {
			selectedCaseDateTypeRow = null;
			caseDateTypeCardsContainer.getChildren().clear();
			updateCaseDateTypeActionState(null);
			return;
		}
		...
		try {
			...
			setCaseDateTypeMessage(rootMessage(ex));
			return;
		}
		caseDateTypeCardsContainer.getChildren().setAll(loadingLabel("Loading case date types…"));
		settingsLoadExecutor.submit(() ->
		{
			try {
				...
				List<CaseDateTypeViewRow> rows = buildCaseDateTypeRows(types, tenantId, mappings);
				Platform.runLater(() ->
				{
					applyCaseDateTypeRows(generation, rows, successMessage);
					renderCaseDateRoleMappings(mappings, types);
				});
			} catch (RuntimeException ex) {
				Platform.runLater(() ->
				{
					if (generation != caseDateTypeLoadGeneration)
						return;
					selectedCaseDateTypeRow = null;
					updateCaseDateTypeActionState(null);
					caseDateTypeCardsContainer.getChildren().setAll(loadingLabel("Failed to load case date types. " + rootMessage(ex)));
				});
			}
		});
	}

	static List<CaseDateTypeViewRow> buildCaseDateTypeRows(List<EffectiveCaseDateTypeDto> rows, int tenantId) {
		return buildCaseDateTypeRows(rows, tenantId, List.of());
	}

	static List<CaseDateTypeViewRow> buildCaseDateTypeRows(List<EffectiveCaseDateTypeDto> rows, int tenantId, List<CaseDateSemanticRoleMappingDto> mappings) {
		Map<Integer, String> roles = new java.util.HashMap<>();
		for (CaseDateSemanticRoleMappingDto m : mappings == null ? List.<CaseDateSemanticRoleMappingDto>of() : mappings)
			if (m.tenantOverride())
				roles.put(m.effectiveTypeId(), m.roleName());
		List<CaseDateTypeViewRow> out = new ArrayList<>();
		for (EffectiveCaseDateTypeDto r : rows == null ? List.<EffectiveCaseDateTypeDto>of() : rows) {
			if (r != null && r.shaleClientId() != null && r.shaleClientId() == tenantId && !r.deleted())
				out.add(new CaseDateTypeViewRow(r, roles.get(r.id())));
		}
		out.sort(java.util.Comparator.comparing(CaseDateTypeViewRow::name, String.CASE_INSENSITIVE_ORDER).thenComparingInt(CaseDateTypeViewRow::id));
		return List.copyOf(out);
	}

	private void applyCaseDateTypeRows(int generation, List<CaseDateTypeViewRow> rows, String successMessage) {
		if (generation != caseDateTypeLoadGeneration)
			return;
		Integer selectedId = selectedCaseDateTypeRow == null ? null : selectedCaseDateTypeRow.id();
		selectedCaseDateTypeRow = preserveCaseDateTypeSelection(rows, selectedId);
		renderCaseDateTypeCards(rows);
		updateCaseDateTypeActionState(selectedCaseDateTypeRow);
		setCaseDateTypeMessage(successMessage == null ? "" : successMessage);
	}

	static CaseDateTypeViewRow preserveCaseDateTypeSelection(List<CaseDateTypeViewRow> rows, Integer selectedId) {
		return rows.stream().filter(row -> selectedId != null && row.id() == selectedId).findFirst().orElse(null);
	}
	...
	private void renderCaseDateTypeCards(List<CaseDateTypeViewRow> rows) {
		if (rows.isEmpty()) {
			caseDateTypeCardsContainer.getChildren().setAll(loadingLabel("No custom Case Date Types yet."));
			return;
		}
		FlowPane grid = new FlowPane(10, 10);
		grid.setPrefWrapLength(760);
		grid.getChildren().setAll(rows.stream().map(this::buildCaseDateTypeCard).toList());
		caseDateTypeCardsContainer.getChildren().setAll(grid);
	}

	private VBox buildCaseDateTypeCard(CaseDateTypeViewRow row) {
		VBox card = new VBox(8);
		card.getStyleClass().addAll("shale-entity-card", "shale-entity-card-compact", "shale-entity-card-selectable", "case-date-custom-card", "shale-density-compact");
		card.setMinWidth(240);
		card.setPrefWidth(340);
		card.setMaxWidth(380);
		card.setUserData(row);
		card.setFocusTraversable(true);
		card.pseudoClassStateChanged(SELECTED_CARD, selectedCaseDateTypeRow != null && selectedCaseDateTypeRow.id() == row.id());
		card.setOnMouseClicked(e ->
		{
			if (e.getButton() == MouseButton.PRIMARY && !isActionControl(e.getTarget()))
				selectCaseDateTypeRow(row);
		});
		...
		Label name = new Label(row.name());
		name.getStyleClass().add("app-dialog-field-label");
		h.getChildren().addAll(dot, name, metadataPill("Custom"));
		FlowPane meta = new FlowPane(6, 6);
		meta.getChildren().addAll(metadataPill(row.active() ? "Active" : "Inactive"), metadataPill(row.category()), metadataPill(row.supportsTime() ? "Timed or all-day"
				: "All-day only"));
		if (row.mappedRoleName() != null)
			meta.getChildren().add(metadataPill("Used for " + row.mappedRoleName()));
		Label help = new Label(row.mappedRoleName() == null ? "Select this type to manage it." : "Change or reset its required-date mapping before deactivating or removing it.");
		help.getStyleClass().add("search-summary-text");
		help.setWrapText(true);
		card.getChildren().addAll(h, meta, help);
		return card;
	}

	private void selectCaseDateTypeRow(CaseDateTypeViewRow row) {
		selectedCaseDateTypeRow = row;
		updateSelectionStyles(caseDateTypeCardsContainer, row.id());
		updateCaseDateTypeActionState(row);
		setCaseDateTypeMessage(row.protectedType() ? PROTECTED_CASE_DATE_TYPE_MESSAGE : "");
	}

	private CaseDateTypeViewRow selectedCaseDateTypeRow() {
		if (selectedCaseDateTypeRow == null)
			setCaseDateTypeMessage("Select a case date type first.");
		return selectedCaseDateTypeRow;
	}
	...
	private void explainProtectedCaseDateType() {
		setCaseDateTypeMessage(PROTECTED_CASE_DATE_TYPE_MESSAGE);
	}

	private void updateCaseDateTypeActionState(CaseDateTypeViewRow row) {
		boolean editable = row != null && row.canEdit();
		boolean toggle = row != null && row.canToggleActive();
		boolean remove = row != null && row.canRemove();
		if (editCaseDateTypeButton != null) {
			editCaseDateTypeButton.setDisable(!editable);
			editCaseDateTypeButton.setTooltip(new Tooltip(row != null && row.protectedType() ? PROTECTED_CASE_DATE_TYPE_MESSAGE : "Edit the selected custom Case Date Type."));
		}
		if (toggleCaseDateTypeButton != null) {
			toggleCaseDateTypeButton.setDisable(!toggle);
			toggleCaseDateTypeButton.setText(row == null ? "Activate/Deactivate" : row.active() ? "Deactivate" : "Activate");
			toggleCaseDateTypeButton.setTooltip(new Tooltip(row != null && row.protectedType() ? PROTECTED_CASE_DATE_TYPE_MESSAGE
					: "Change whether the selected custom type is available for new Case Dates."));
		}
		if (removeCaseDateTypeButton != null) {
			removeCaseDateTypeButton.setDisable(!remove);
			removeCaseDateTypeButton.setText("Remove");
			removeCaseDateTypeButton.setTooltip(new Tooltip(row != null && row.protectedType() ? PROTECTED_CASE_DATE_TYPE_MESSAGE
					: "Remove the selected custom type from future selections."));
		}
	}

	private Optional<CaseDateTypeInput> showCaseDateTypeDialog(EffectiveCaseDateTypeDto existing) {
		Dialog<CaseDateTypeInput> dialog = new Dialog<>();
		String title = existing == null ? "Add Case Date Type" : "Edit Case Date Type";
		...
		TextField description = new TextField(existing == null ? "" : safe(existing.description()));
		...
		category.getSelectionModel().select(existing == null ? "OTHER" : existing.calendarCategory());
		ControlStyles.formControl(category);
		...
		CheckBox supports = new CheckBox("Supports time of day");
		supports.setSelected(existing != null && existing.supportsTime());
		...
		dialog.setResultConverter(b ->
		{
			if (b != ButtonType.OK)
				return null;
			return new CaseDateTypeInput(trim(name.getText()), trim(description.getText()), category.getValue(), fxColorToDb(colorPicker.getValue()), supports.isSelected(),
					existing == null ? null : safe(existing.systemKey()), existing == null ? null : existing.sortOrder(), active.isSelected());
		});
		try {
			return dialog.showAndWait();
		} catch (RuntimeException ex) {
			AppDialogs.showError(dialog.getOwner(), "Case Date Types", rootMessage(ex));
			return Optional.empty();
		}
	}

	private void setCaseDateTypeMessage(String msg) {
		if (caseDateTypeSettingsStatusLabel != null)
			caseDateTypeSettingsStatusLabel.setText(msg == null ? "" : msg);
	}

	private void showCaseDateTypeError(RuntimeException ex) {
		LOG.error("Case Date Type administration mutation failed", ex);
		AppDialogs.showError(caseDateTypeCardsContainer == null || caseDateTypeCardsContainer.getScene() == null ? null : caseDateTypeCardsContainer.getScene().getWindow(),
				"Case Date Types", "The Case Date Type could not be saved. Contact an administrator if the problem continues.");
	}

	public record CaseDateTypeViewRow(EffectiveCaseDateTypeDto type, String mappedRoleName) {
		int id() {
			return type.id();
		}

		String name() {
			return safe(type.name());
		}

		String color() {
			return safe(type.color());
		}

		String category() {
			return safe(type.calendarCategory());
		}

		boolean supportsTime() {
			return type.supportsTime();
		}
	...
	@FXML
	private void onMoveCaseStatusUp() {
		if (requireAdminLookupManagement("Case Statuses"))
			moveSelectedStatus(-1);
	}

	@FXML
	private void onMoveCaseStatusDown() {
		if (requireAdminLookupManagement("Case Statuses"))
			moveSelectedStatus(1);
	}
	...
	private void loadCaseStatusesAsync(String successMessage) {
		if (caseService == null || caseStatusCardsContainer == null)
			return;
		if (!requireAdminLookupManagement("Case Statuses")) {
			caseStatusRows.clear();
		...
		settingsLoadExecutor.submit(() ->
		{
			try {
				...
				for (CaseStatusDto status : caseService.listCaseStatuses(tenantId, true))
					rows.add(new CaseStatusViewRow(status));
	...
	private VBox buildCaseStatusCard(CaseStatusViewRow row, int index) {
		...
		if (!row.getLifecycleKey().isBlank())
			metadata.getChildren().add(metadataPill("Lifecycle: " + row.getLifecycleKey()));
		if (!row.getSystemKey().isBlank())
			metadata.getChildren().add(metadataPill("System: " + row.getSystemKey()));
		...
		Button lifecycle = cardButton(row.active() && !row.deleted() ? "Remove" : "Restore", row.active() && !row.deleted() ? ControlStyles.Purpose.DANGER
				: ControlStyles.Purpose.SECONDARY);
		lifecycle.setDisable(row.global());
		lifecycle.setOnAction(event ->
		{
			selectCaseStatusRow(row);
			if (row.active() && !row.deleted())
				caseService.removeCaseStatus(new CaseServicePort.StatusLifecycleCommand(requireTenantId(), requireActorUserId(), row.id()));
			else
				caseService.restoreCaseStatus(new CaseServicePort.StatusLifecycleCommand(requireTenantId(), requireActorUserId(), row.id()));
			loadCaseStatusesAsync(row.active() && !row.deleted() ? "Case status removed." : "Case status restored.");
			event.consume();
		});
		Label restriction = new Label(row.global() ? "Global/default status: editing creates a tenant override; reordering requires tenant-specific status."
				: "Tenant-specific/custom status.");
		restriction.getStyleClass().add("search-summary-text");
	...
	private void styleLookupDialog(Dialog<?> dialog, Control... controls) {
		...
		if (ok instanceof ButtonBase button)
			ControlStyles.apply(button, ControlStyles.Purpose.PRIMARY);
		if (cancel instanceof ButtonBase button)
			ControlStyles.apply(button, ControlStyles.Purpose.SECONDARY);
	}
	...
	private void moveSelectedStatus(int delta) {
		...
		if (otherIndex < 0 || otherIndex >= caseStatusRows.size())
			return;
	...
	private Optional<CaseStatusInput> showCaseStatusDialog(CaseStatusDto existing) {
		...
		try {
			...
			AppDialogs.showError(dialog.getOwner(), "Case Statuses", rootMessage(ex));
			return Optional.empty();
		}
	}

	private CaseStatusViewRow selectedStatusRow() {
		if (selectedCaseStatusRow == null)
			setCaseStatusMessage("Select a case status first.");
	...
	private void setCaseStatusMessage(String message) {
		if (caseStatusSettingsStatusLabel != null)
			caseStatusSettingsStatusLabel.setText(message == null ? "" : message);
	}
	...
	@FXML
	private void onRemoveUserFromTenant() {
		UserManagementViewRow selected = selectedManagedUser();
		if (selected == null || userMutationRunning)
			return;
		...
		if (!AppDialogs.showConfirmation(null, "Remove from Tenant", "Remove " + selected.name() + " from this tenant?", warning, "Remove from Tenant",
				AppDialogs.DialogActionKind.DANGER))
			return;
		userMutationRunning = true;
		updateUserActionButtons(selected);
		setUserManagementMessage("Removing user from tenant…");
		int removedId = selected.id();
		int generation = ++userManagementLoadGeneration;
		settingsLoadExecutor.submit(() ->
		{
			try {
				userDao.removeUserFromTenant(removedId, selected.rowVer());
				Platform.runLater(() ->
				{
					if (generation != userManagementLoadGeneration)
						return;
					userMutationRunning = false;
					managedUserRows.removeIf(r -> r.id() == removedId);
					applyUserFilter();
					userManagementTable.getSelectionModel().clearSelection();
					updateUserActionButtons(null);
					setUserManagementMessage("User removed from tenant.");
					loadManagedUsersAsync("User removed from tenant.");
				});
			} catch (RuntimeException ex) {
				Platform.runLater(() ->
				{
					...
					userMutationRunning = false;
					updateUserActionButtons(selected);
					AppDialogs.showError(null, "Remove from Tenant", rootMessage(ex));
					setUserManagementMessage(rootMessage(ex));
				});
			}
		});
	}
	...
	private void onEditUser() {
		...
		if (selected == null || userMutationRunning)
			return;
		showEditUserDialog(selected).ifPresent(request ->
		{
			userMutationRunning = true;
			updateUserActionButtons(selected);
			setUserManagementMessage("Saving changes…");
			settingsLoadExecutor.submit(() ->
			{
				try {
					var result = userDao.updateManagedUser(request);
					Platform.runLater(() ->
					{
						userMutationRunning = false;
						loadManagedUsersAsync(result.changed() ? "User updated." : "No changes to save.");
					});
				} catch (RuntimeException ex) {
					Platform.runLater(() ->
					{
						userMutationRunning = false;
						updateUserActionButtons(selected);
						AppDialogs.showError(null, "Edit User", rootMessage(ex));
						setUserManagementMessage(rootMessage(ex));
					});
				}
			});
	...
	private void onDeactivateUser() {
		...
		if (selected == null)
			return;
		boolean confirmed = AppDialogs.showConfirmation(null, "Deactivate User", "Deactivate this user?", "This will disable their access while preserving historical records.",
				"Deactivate", AppDialogs.DialogActionKind.DANGER);
		if (!confirmed)
			return;
		try {
	...
	private void loadManagedUsersAsync(String successMessage) {
		if (userDao == null || userManagementTable == null || !isAdminUser())
			return;
	...
	private void setMaterialTypeMessage(String message) {
		if (materialTypeSettingsStatusLabel != null)
			materialTypeSettingsStatusLabel.setText(message == null ? "" : message);
	}

	private void setRequestMethodMessage(String message) {
		if (requestMethodSettingsStatusLabel != null)
			requestMethodSettingsStatusLabel.setText(message == null ? "" : message);
	}

	private void setRequestStatusMessage(String message) {
		if (requestStatusSettingsStatusLabel != null)
			requestStatusSettingsStatusLabel.setText(message == null ? "" : message);
	}
	...
	public static final class CaseStatusViewRow {
		...
		CaseStatusViewRow(CaseStatusDto status) {
			this.status = status;
		}
		...
		public String getLifecycleKey() {
			return safe(status.lifecycleKey());
		}

		public String getSystemKey() {
			return safe(status.systemKey());
		}
		...
		public boolean global() {
			return status.shaleClientId() == null;
		}
	...
	private RequestLookupSelection selectedRequestLookup(RequestLookupKind kind) {
		return switch (kind) {
		case MATERIAL_TYPE -> selectedMaterialTypeRow;
		case REQUEST_METHOD -> selectedRequestMethodRow;
		case REQUEST_STATUS -> selectedRequestStatusRow;
		};
	}

	private void selectRequestLookup(RequestLookupKind kind, RequestLookupSelection row) {
		switch (kind) {
		case MATERIAL_TYPE -> selectedMaterialTypeRow = row;
		case REQUEST_METHOD -> selectedRequestMethodRow = row;
		case REQUEST_STATUS -> selectedRequestStatusRow = row;
		}
	...
	private void updateRequestLookupSelectionStyles(RequestLookupKind kind) {
		VBox container = switch (kind) {
		case MATERIAL_TYPE -> materialTypeCardsContainer;
		case REQUEST_METHOD -> requestMethodCardsContainer;
		case REQUEST_STATUS -> requestStatusCardsContainer;
		};
		if (container == null)
			return;
		for (Node node : container.getChildren()) {
			if (node.getUserData() instanceof RequestLookupSelection row)
				node.pseudoClassStateChanged(SELECTED_CARD, isSelectedRequestLookup(kind, row));
		}
	}
	...
	private void onAddMaterialType() {
		addRequestLookup(RequestLookupKind.MATERIAL_TYPE);
	}

	private void onEditMaterialType() {
		editRequestLookup(RequestLookupKind.MATERIAL_TYPE);
	}

	private void onToggleMaterialTypeActive() {
		toggleRequestLookup(RequestLookupKind.MATERIAL_TYPE);
	}

	private void onResetOrRemoveMaterialType() {
		resetRequestLookup(RequestLookupKind.MATERIAL_TYPE);
	}

	private void onAddRequestMethod() {
		addRequestLookup(RequestLookupKind.REQUEST_METHOD);
	}

	private void onEditRequestMethod() {
		editRequestLookup(RequestLookupKind.REQUEST_METHOD);
	}

	private void onToggleRequestMethodActive() {
		toggleRequestLookup(RequestLookupKind.REQUEST_METHOD);
	}

	private void onResetOrRemoveRequestMethod() {
		resetRequestLookup(RequestLookupKind.REQUEST_METHOD);
	}

	private void onAddRequestStatus() {
		addRequestLookup(RequestLookupKind.REQUEST_STATUS);
	}

	private void onEditRequestStatus() {
		editRequestLookup(RequestLookupKind.REQUEST_STATUS);
	}

	private void onToggleRequestStatusActive() {
		toggleRequestLookup(RequestLookupKind.REQUEST_STATUS);
	}

	private void onResetOrRemoveRequestStatus() {
		resetRequestLookup(RequestLookupKind.REQUEST_STATUS);
	}

	private void addRequestLookup(RequestLookupKind kind) {
		if (!requireAdminLookupManagement(label(kind)) || mutationRunning(kind))
			return;
		showRequestLookupDialog(kind, null).ifPresent(input -> mutateRequestLookup(kind, () ->
		{
			switch (kind) {
	...
	private void editRequestLookup(RequestLookupKind kind) {
		RequestLookupSelection row = selectedRequestLookup(kind);
		if (row == null) {
			message(kind, "Select a " + label(kind).toLowerCase(java.util.Locale.ROOT) + " first.");
			return;
		}
		if (mutationRunning(kind))
			return;
		showRequestLookupDialog(kind, row).ifPresent(input -> mutateRequestLookup(kind, () ->
		{
			switch (kind) {
	...
	private void toggleRequestLookup(RequestLookupKind kind) {
		...
		mutateRequestLookup(kind, () ->
		{
			var c = new MaterialRequestServicePort.SetLookupActiveCommand(requireTenantId(), requireActorUserId(), row.id(), !row.active(), row.rowVer());
			switch (kind) {
			case MATERIAL_TYPE -> materialRequestService.setMaterialTypeActive(c);
			case REQUEST_METHOD -> materialRequestService.setRequestMethodActive(c);
			case REQUEST_STATUS -> materialRequestService.setRequestStatusActive(c);
			}
		}, row.active() ? label(kind) + " deactivated for future selections." : label(kind) + " activated.");
	}

	private void resetRequestLookup(RequestLookupKind kind) {
		RequestLookupSelection row = selectedRequestLookup(kind);
		if (row == null) {
			message(kind, "Select a " + label(kind).toLowerCase(java.util.Locale.ROOT) + " first.");
			return;
		}
		String action = row.custom() ? "Remove" : "Reset to Default";
		if (row.global()) {
			message(kind, "Global defaults do not need reset.");
			return;
		}
		if (!AppDialogs.showConfirmation(null, label(kind), action + " " + row.name() + "?", "Existing records retain their stored lookup relationship.", action, row.custom()
				? AppDialogs.DialogActionKind.DANGER
				: AppDialogs.DialogActionKind.PRIMARY))
			return;
		mutateRequestLookup(kind, () ->
		{
			var c = new MaterialRequestServicePort.ResetLookupOverrideCommand(requireTenantId(), requireActorUserId(), row.id());
			switch (kind) {
			case MATERIAL_TYPE -> materialRequestService.resetMaterialTypeOverride(c);
			case REQUEST_METHOD -> materialRequestService.resetRequestMethodOverride(c);
			case REQUEST_STATUS -> materialRequestService.resetRequestStatusOverride(c);
			}
		}, row.custom() ? label(kind) + " removed from future selections." : "Tenant override reset to global default.");
	}

	private Optional<RequestLookupInput> showRequestLookupDialog(RequestLookupKind kind, RequestLookupSelection existing) {
		Dialog<RequestLookupInput> d = new Dialog<>();
		String title = (existing == null ? "Add " : existing.global() ? "Customize " : "Edit ") + label(kind);
		d.setTitle(title);
		AppDialogs.applySecondaryDialogShell(d, title);
		...
		TextField sort = new TextField(existing == null ? "0" : String.valueOf(existing.sortOrder()));
		CheckBox active = new CheckBox("Active");
		active.setSelected(existing == null || existing.active());
		ColorPicker colorPicker = new ColorPicker(dbColorToFx(existing == null ? null : existing.color()));
		...
		grid.add(name, 1, 0);
		int r = 1;
		if (kind == RequestLookupKind.MATERIAL_TYPE) {
			grid.add(new Label("Description"), 0, r);
			grid.add(description, 1, r++);
		}
		grid.add(new Label("Color"), 0, r);
		grid.add(colorPicker, 1, r++);
		if (kind != RequestLookupKind.REQUEST_METHOD) {
			grid.add(new Label("Sort Order"), 0, r);
			grid.add(sort, 1, r++);
		}
		grid.add(active, 1, r++);
		if (existing != null && !existing.systemKey().isBlank()) {
			grid.add(new Label("System Key"), 0, r);
			grid.add(new Label(existing.systemKey()), 1, r);
		}
		d.getDialogPane().setContent(grid);
		styleLookupDialog(d, name, description, sort, colorPicker, active);
		d.setResultConverter(b ->
		{
			if (b != ButtonType.OK)
				return null;
			...
			if (kind != RequestLookupKind.REQUEST_METHOD) {
				try {
					...
					throw new IllegalArgumentException("Sort Order must be a number.");
				}
			}
			return new RequestLookupInput(nm, kind == RequestLookupKind.MATERIAL_TYPE ? trim(description.getText()) : null, fxColorToDb(colorPicker.getValue()), active
					.isSelected(), existing == null ? null : existing.systemKey(), sortOrder);
		});
		try {
			return d.showAndWait();
		} catch (RuntimeException ex) {
			AppDialogs.showError(null, label(kind), rootMessage(ex));
			return Optional.empty();
		}
	}

	private void mutateRequestLookup(RequestLookupKind kind, Runnable work, String success) {
		if (mutationRunning(kind))
			return;
		setMutationRunning(kind, true);
		message(kind, "Saving…");
		settingsLoadExecutor.submit(() ->
		{
			try {
				...
				Platform.runLater(() ->
				{
					setMutationRunning(kind, false);
					AppDialogs.showError(null, label(kind), rootMessage(ex));
					message(kind, rootMessage(ex));
				});
			}
		});
	}

	private boolean mutationRunning(RequestLookupKind kind) {
		return switch (kind) {
		case MATERIAL_TYPE -> materialTypeMutationRunning;
		case REQUEST_METHOD -> requestMethodMutationRunning;
		case REQUEST_STATUS -> requestStatusMutationRunning;
		};
	}

	private void setMutationRunning(RequestLookupKind kind, boolean v) {
		switch (kind) {
		case MATERIAL_TYPE -> materialTypeMutationRunning = v;
		case REQUEST_METHOD -> requestMethodMutationRunning = v;
		case REQUEST_STATUS -> requestStatusMutationRunning = v;
		}
	}

	private void message(RequestLookupKind kind, String m) {
		switch (kind) {
		case MATERIAL_TYPE -> setMaterialTypeMessage(m);
		case REQUEST_METHOD -> setRequestMethodMessage(m);
		case REQUEST_STATUS -> setRequestStatusMessage(m);
		}
	}

	private static String label(RequestLookupKind kind) {
		return switch (kind) {
		case MATERIAL_TYPE -> "Material Type";
		case REQUEST_METHOD -> "Request Method";
		case REQUEST_STATUS -> "Request Status";
		};
	}

	static List<MaterialTypeDto> buildMaterialTypeRows(List<MaterialTypeDto> rows, int tenantId) {
		Map<String, MaterialTypeDto> g = new java.util.LinkedHashMap<>(), t = new java.util.LinkedHashMap<>();
		List<MaterialTypeDto> out = new ArrayList<>();
		for (MaterialTypeDto r : rows == null ? List.<MaterialTypeDto>of() : rows) {
			if (r.shaleClientId() != null && r.shaleClientId() != tenantId)
				continue;
			String k = safe(r.systemKey()).toLowerCase(java.util.Locale.ROOT);
			if (r.shaleClientId() == null && !k.isBlank())
				g.put(k, r);
			else if (!k.isBlank())
				t.put(k, r);
			else if (!r.deleted())
		...
		for (var e : g.entrySet()) {
			MaterialTypeDto o = t.get(e.getKey());
			out.add(o == null || o.deleted() ? e.getValue() : o);
		}
		for (var e : t.entrySet())
			if (!g.containsKey(e.getKey()) && !e.getValue().deleted())
				out.add(e.getValue());
		out.sort(java.util.Comparator.comparing(MaterialTypeDto::sortOrder).thenComparing(MaterialTypeDto::name, String.CASE_INSENSITIVE_ORDER));
		return List.copyOf(out);
	}

	static List<RequestMethodDto> buildRequestMethodRows(List<RequestMethodDto> rows, int tenantId) {
		Map<String, RequestMethodDto> g = new java.util.LinkedHashMap<>(), t = new java.util.LinkedHashMap<>();
		List<RequestMethodDto> out = new ArrayList<>();
		for (RequestMethodDto r : rows == null ? List.<RequestMethodDto>of() : rows) {
			if (r.shaleClientId() != null && r.shaleClientId() != tenantId)
				continue;
			String k = safe(r.systemKey()).toLowerCase(java.util.Locale.ROOT);
			if (r.shaleClientId() == null && !k.isBlank())
		...
		for (var e : g.entrySet()) {
			RequestMethodDto o = t.get(e.getKey());
			out.add(o == null || o.deleted() ? e.getValue() : o);
		}
		for (var e : t.entrySet())
			if (!g.containsKey(e.getKey()) && !e.getValue().deleted())
				out.add(e.getValue());
		out.sort(java.util.Comparator.comparing(RequestMethodDto::sortOrder).thenComparing(RequestMethodDto::name, String.CASE_INSENSITIVE_ORDER));
		return List.copyOf(out);
	}

	static List<RequestStatusDto> buildRequestStatusRows(List<RequestStatusDto> rows, int tenantId) {
		Map<String, RequestStatusDto> g = new java.util.LinkedHashMap<>(), t = new java.util.LinkedHashMap<>();
		List<RequestStatusDto> out = new ArrayList<>();
		for (RequestStatusDto r : rows == null ? List.<RequestStatusDto>of() : rows) {
			if (r.shaleClientId() != null && r.shaleClientId() != tenantId)
				continue;
			String k = safe(r.systemKey()).toLowerCase(java.util.Locale.ROOT);
			if (r.shaleClientId() == null && !k.isBlank())
		...
		for (var e : g.entrySet()) {
			RequestStatusDto o = t.get(e.getKey());
			out.add(o == null || o.deleted() ? e.getValue() : o);
		}
		for (var e : t.entrySet())
			if (!g.containsKey(e.getKey()) && !e.getValue().deleted())
				out.add(e.getValue());
		out.sort(java.util.Comparator.comparing(RequestStatusDto::sortOrder).thenComparing(RequestStatusDto::name, String.CASE_INSENSITIVE_ORDER));
		return List.copyOf(out);
	}
	...
	private record RequestLookupSelection(int id, String name, String description, String color, String systemKey, int sortOrder, boolean active, boolean global, byte[] rowVer) {
		static RequestLookupSelection material(MaterialTypeDto d) {
			return new RequestLookupSelection(d.id(), safe(d.name()), safe(d.description()), safe(d.color()), safe(d.systemKey()), d.sortOrder(), d.active() && !d.deleted(), d
					.shaleClientId() == null, d.rowVer());
		}

		static RequestLookupSelection method(RequestMethodDto d) {
			return new RequestLookupSelection(d.id(), safe(d.name()), "", safe(d.color()), safe(d.systemKey()), d.sortOrder(), d.active() && !d.deleted(), d
					.shaleClientId() == null, d.rowVer());
		}

		static RequestLookupSelection status(RequestStatusDto d) {
			return new RequestLookupSelection(d.id(), safe(d.name()), "", safe(d.color()), safe(d.systemKey()), d.sortOrder(), d.active() && !d.deleted(), d
					.shaleClientId() == null, d.rowVer());
		}

		boolean custom() {
			return !global && systemKey.isBlank();
		}

		String scopeLabel() {
			return global ? "Global/default" : custom() ? "Tenant custom" : "Tenant override";
		}

		String lifecycleText() {
			return global ? "Editing or changing active state creates a tenant override; global rows are never changed."
					: custom() ? "Tenant custom value can be edited, activated/deactivated, or removed." : "Tenant override masks the global default until reset.";
		}
	}
