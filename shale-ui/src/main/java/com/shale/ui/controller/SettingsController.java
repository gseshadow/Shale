...
public final class SettingsController {
	...
	@FXML private void onToggleCaseDateTypeActive(){ if(!requireAdminLookupManagement("Case Date Types"))return; CaseDateTypeViewRow selected=selectedCaseDateTypeRow(); if(selected==null)return; try{caseService.setCaseDateTypeActive(new CaseServicePort.SetCaseDateTypeActiveCommand(requireTenantId(),requireActorUserId(),selected.id(),!selected.active(),selected.rowVer())); publishCaseDateTypeChanged(selected.id()); loadCaseDateTypesAsync(selected.active()?"Case date type deactivated for new occurrences.":"Case date type activated.");}catch(RuntimeException ex){showCaseDateTypeError(ex);} }
	...
	private VBox buildCaseDateTypeCard(CaseDateTypeViewRow row){VBox card=new VBox(8);card.getStyleClass().addAll("shale-entity-card","shale-entity-card-compact","shale-entity-card-selectable","shale-density-compact");card.setOnMouseClicked(e->{if(e.getButton()==MouseButton.PRIMARY&&!isActionControl(e.getTarget()))selectCaseDateTypeRow(row);});HBox h=new HBox(10);h.setAlignment(Pos.CENTER_LEFT);Circle dot=new Circle(6);String css=safe(ColorUtil.toCssBackgroundColorOrNull(row.color()));if(!css.isBlank())dot.setStyle("-fx-background-color: "+css+"; -fx-fill: "+css+";");Label name=new Label(row.name());name.getStyleClass().add("app-dialog-field-label");h.getChildren().addAll(dot,name,metadataPill(row.category()),metadataPill(row.supportsTime()?"Timed or all-day":"All-day only"));HBox meta=new HBox(6);meta.getChildren().addAll(metadataPill(row.active()?"Active":"Inactive"),metadataPill(row.scopeLabel()),metadataPill(row.protectedType()?"Protected system type":"Custom type"));HBox actions=new HBox(8);Button edit=cardButton("Edit",ControlStyles.Purpose.GHOST);edit.setDisable(row.protectedType());edit.setOnAction(e->{selectCaseDateTypeRow(row);onEditCaseDateType();});Button toggle=cardButton(row.active()?"Deactivate":"Activate",ControlStyles.Purpose.GHOST);toggle.setDisable(row.protectedType());toggle.setOnAction(e->{selectCaseDateTypeRow(row);onToggleCaseDateTypeActive();});Button reset=cardButton(row.custom()?"Remove":"Reset to Default",row.custom()?ControlStyles.Purpose.DANGER:ControlStyles.Purpose.SECONDARY);reset.setDisable(row.protectedType());reset.setOnAction(e->{selectCaseDateTypeRow(row);onResetOrRemoveCaseDateType();});actions.getChildren().addAll(edit,toggle,reset);card.getChildren().addAll(h,meta,actions);return card;}
