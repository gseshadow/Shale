...
public final class SettingsController {
	...
	private void addRequestLookup(RequestLookupKind kind){ if (!requireAdminLookupManagement(label(kind)) || mutationRunning(kind)) return; showRequestLookupDialog(kind,null).ifPresent(input -> mutateRequestLookup(kind, () -> { switch(kind){ case MATERIAL_TYPE -> materialRequestService.createMaterialType(new MaterialRequestServicePort.MaterialTypeCommand(null,requireTenantId(),requireActorUserId(),input.name(),input.description(),input.color(),input.active(),input.systemKey(),input.sortOrder(),null)); case REQUEST_METHOD -> materialRequestService.createRequestMethod(new MaterialRequestServicePort.RequestMethodCommand(null,requireTenantId(),requireActorUserId(),input.name(),input.color(),input.active(),input.systemKey(),input.sortOrder(),null)); case REQUEST_STATUS -> materialRequestService.createRequestStatus(new MaterialRequestServicePort.RequestStatusCommand(null,requireTenantId(),requireActorUserId(),input.name(),input.color(),input.active(),input.systemKey(),input.sortOrder(),null)); } }, label(kind)+" added.")); }
	private void editRequestLookup(RequestLookupKind kind){ RequestLookupSelection row=selectedRequestLookup(kind); if(row==null){message(kind,"Select a "+label(kind).toLowerCase(java.util.Locale.ROOT)+" first.");return;} if(mutationRunning(kind))return; showRequestLookupDialog(kind,row).ifPresent(input -> mutateRequestLookup(kind, () -> { switch(kind){ case MATERIAL_TYPE -> materialRequestService.updateMaterialType(new MaterialRequestServicePort.MaterialTypeCommand(row.id(),requireTenantId(),requireActorUserId(),input.name(),input.description(),input.color(),input.active(),row.systemKey().isBlank()?input.systemKey():row.systemKey(),input.sortOrder(),row.rowVer())); case REQUEST_METHOD -> materialRequestService.updateRequestMethod(new MaterialRequestServicePort.RequestMethodCommand(row.id(),requireTenantId(),requireActorUserId(),input.name(),input.color(),input.active(),row.systemKey().isBlank()?input.systemKey():row.systemKey(),input.sortOrder(),row.rowVer())); case REQUEST_STATUS -> materialRequestService.updateRequestStatus(new MaterialRequestServicePort.RequestStatusCommand(row.id(),requireTenantId(),requireActorUserId(),input.name(),input.color(),input.active(),row.systemKey().isBlank()?input.systemKey():row.systemKey(),input.sortOrder(),row.rowVer())); } }, row.global()?"Tenant override saved.":label(kind)+" updated.")); }
