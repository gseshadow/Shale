...
public final class SettingsController {
	...
	private void configureLookupActionRows() {
		...
		if (caseDateTypeActionRow != null) configureRequestActionRow(caseDateTypeActionRow, "Case Date Type", caseDateTypeSettingsStatusLabel, this::onAddCaseDateType, this::onEditCaseDateType, this::onToggleCaseDateTypeActive, this::onResetOrRemoveCaseDateType);
	...
	@FXML private void onAddCaseDateType(){ if(!requireAdminLookupManagement("Case Date Types"))return; showCaseDateTypeDialog(null).ifPresent(input->{try{caseService.createCaseDateType(new CaseServicePort.CaseDateTypeCommand(null,requireTenantId(),requireActorUserId(),input.systemKey(),input.name(),input.description(),input.category(),input.color(),input.supportsTime(),input.sortOrder(),input.active(),null)); loadCaseDateTypesAsync("Case date type added.");}catch(RuntimeException ex){showCaseDateTypeError(ex);}}); }
	@FXML private void onEditCaseDateType(){ if(!requireAdminLookupManagement("Case Date Types"))return; CaseDateTypeViewRow selected=selectedCaseDateTypeRow(); if(selected==null)return; showCaseDateTypeDialog(selected.type()).ifPresent(input->{try{caseService.updateCaseDateType(new CaseServicePort.CaseDateTypeCommand(selected.id(),requireTenantId(),requireActorUserId(),selected.systemKeyForSave(),input.name(),input.description(),input.category(),input.color(),input.supportsTime(),input.sortOrder(),input.active(),selected.rowVer())); loadCaseDateTypesAsync(selected.global()?"Tenant override saved for global case date type.":"Case date type updated.");}catch(RuntimeException ex){showCaseDateTypeError(ex);}}); }
	@FXML private void onToggleCaseDateTypeActive(){ if(!requireAdminLookupManagement("Case Date Types"))return; CaseDateTypeViewRow selected=selectedCaseDateTypeRow(); if(selected==null)return; try{caseService.setCaseDateTypeActive(new CaseServicePort.SetCaseDateTypeActiveCommand(requireTenantId(),requireActorUserId(),selected.id(),!selected.active(),selected.rowVer())); loadCaseDateTypesAsync(selected.active()?"Case date type deactivated for new occurrences.":"Case date type activated.");}catch(RuntimeException ex){showCaseDateTypeError(ex);} }
	@FXML private void onResetOrRemoveCaseDateType(){ if(!requireAdminLookupManagement("Case Date Types"))return; CaseDateTypeViewRow selected=selectedCaseDateTypeRow(); if(selected==null)return; String action=selected.custom()?"Remove":"Reset to Default"; if(!AppDialogs.showConfirmation(caseDateTypeCardsContainer==null||caseDateTypeCardsContainer.getScene()==null?null:caseDateTypeCardsContainer.getScene().getWindow(),"Case Date Types",action+" "+selected.name()+"?","This affects future type selections only. Existing Case Dates retain their stored type presentation.",action,selected.custom()?AppDialogs.DialogActionKind.DANGER:AppDialogs.DialogActionKind.PRIMARY))return; try{caseService.resetCaseDateTypeOverride(new CaseServicePort.ResetCaseDateTypeOverrideCommand(requireTenantId(),requireActorUserId(),selected.id())); loadCaseDateTypesAsync(selected.custom()?"Custom case date type removed from future selections.":"Tenant override reset to global default.");}catch(RuntimeException ex){showCaseDateTypeError(ex);} }
	private void loadCaseDateTypesAsync(String successMessage){ if(caseService==null||caseDateTypeCardsContainer==null)return; if(!requireAdminLookupManagement("Case Date Types")){caseDateTypeCardsContainer.getChildren().clear();return;} final int generation=++caseDateTypeLoadGeneration; final int tenantId; final int actorUserId; try{tenantId=requireTenantId();actorUserId=requireActorUserId();}catch(RuntimeException ex){setCaseDateTypeMessage(rootMessage(ex));return;} caseDateTypeCardsContainer.getChildren().setAll(loadingLabel("Loading case date types…")); settingsLoadExecutor.submit(()->{try{List<CaseDateTypeViewRow> rows=buildCaseDateTypeRows(caseService.listCaseDateTypesForAdministration(tenantId,actorUserId),tenantId); Platform.runLater(()->{if(generation!=caseDateTypeLoadGeneration)return; renderCaseDateTypeCards(rows); setCaseDateTypeMessage(successMessage==null?"":successMessage);});}catch(RuntimeException ex){Platform.runLater(()->{if(generation!=caseDateTypeLoadGeneration)return; caseDateTypeCardsContainer.getChildren().setAll(loadingLabel("Failed to load case date types. "+rootMessage(ex)));});}}); }
	...
	private VBox buildCaseDateTypeCard(CaseDateTypeViewRow row){VBox card=new VBox(8);card.getStyleClass().addAll("shale-entity-card","shale-entity-card-compact","shale-entity-card-selectable","shale-density-compact");card.setOnMouseClicked(e->{if(e.getButton()==MouseButton.PRIMARY&&!isActionControl(e.getTarget()))selectCaseDateTypeRow(row);});HBox h=new HBox(10);h.setAlignment(Pos.CENTER_LEFT);Circle dot=new Circle(6);String css=safe(ColorUtil.toCssBackgroundColorOrNull(row.color()));if(!css.isBlank())dot.setStyle("-fx-background-color: "+css+"; -fx-fill: "+css+";");Label name=new Label(row.name());name.getStyleClass().add("app-dialog-field-label");h.getChildren().addAll(dot,name,metadataPill(row.category()),metadataPill(row.supportsTime()?"Timed or all-day":"All-day only"));HBox meta=new HBox(6);meta.getChildren().addAll(metadataPill(row.active()?"Active":"Inactive"),metadataPill(row.scopeLabel()));HBox actions=new HBox(8);Button edit=cardButton(row.global()?"Customize":"Edit",ControlStyles.Purpose.GHOST);edit.setOnAction(e->{selectCaseDateTypeRow(row);onEditCaseDateType();});Button toggle=cardButton(row.active()?"Deactivate":"Activate",ControlStyles.Purpose.GHOST);toggle.setOnAction(e->{selectCaseDateTypeRow(row);onToggleCaseDateTypeActive();});Button reset=cardButton(row.custom()?"Remove":"Reset to Default",row.custom()?ControlStyles.Purpose.DANGER:ControlStyles.Purpose.SECONDARY);reset.setDisable(row.global());reset.setOnAction(e->{selectCaseDateTypeRow(row);onResetOrRemoveCaseDateType();});actions.getChildren().addAll(edit,toggle,reset);card.getChildren().addAll(h,meta,actions);return card;}
	private void selectCaseDateTypeRow(CaseDateTypeViewRow row){selectedCaseDateTypeRow=row;updateSelectionStyles(caseDateTypeCardsContainer,row.id());}
	...
	private Optional<CaseDateTypeInput> showCaseDateTypeDialog(EffectiveCaseDateTypeDto existing){Dialog<CaseDateTypeInput> dialog=new Dialog<>();String title=existing==null?"Add Case Date Type":(existing.shaleClientId()==null?"Customize Case Date Type":"Edit Case Date Type");dialog.setTitle(title);AppDialogs.applySecondaryDialogShell(dialog,title);dialog.getDialogPane().getButtonTypes().setAll(ButtonType.OK,ButtonType.CANCEL);TextField name=new TextField(existing==null?"":existing.name());TextField description=new TextField(existing==null?"":safe(existing.description()));ChoiceBox<String> category=new ChoiceBox<>();category.getItems().setAll("DEADLINE","TRIAL","HEARING","MEDIATION","DEPOSITION","NOTICE","APPOINTMENT","MILESTONE","OTHER");category.getSelectionModel().select(existing==null?"OTHER":existing.calendarCategory());ControlStyles.formControl(category);TextField key=new TextField(existing==null?"":safe(existing.systemKey()));ColorPicker colorPicker=new ColorPicker(dbColorToFx(existing==null?null:existing.color()));CheckBox active=new CheckBox("Active");active.setSelected(existing==null||existing.active());CheckBox supports=new CheckBox("Supports time of day");supports.setSelected(existing!=null&&existing.supportsTime());GridPane grid=new GridPane();grid.setHgap(8);grid.setVgap(8);grid.add(new Label("Name"),0,0);grid.add(name,1,0);grid.add(new Label("Description"),0,1);grid.add(description,1,1);grid.add(new Label("Calendar Category"),0,2);grid.add(category,1,2);grid.add(new Label("Color"),0,3);grid.add(colorPicker,1,3);grid.add(supports,1,4);grid.add(active,1,5);if(existing==null){grid.add(new Label("System Key (optional)"),0,6);grid.add(key,1,6);}dialog.getDialogPane().setContent(grid);styleLookupDialog(dialog,name,colorPicker,active);dialog.setResultConverter(b->{if(b!=ButtonType.OK)return null;return new CaseDateTypeInput(trim(name.getText()),trim(description.getText()),category.getValue(),fxColorToDb(colorPicker.getValue()),supports.isSelected(),existing==null?trim(key.getText()):safe(existing.systemKey()),existing==null?null:existing.sortOrder(),active.isSelected());});try{return dialog.showAndWait();}catch(RuntimeException ex){AppDialogs.showError(dialog.getOwner(),"Case Date Types",rootMessage(ex));return Optional.empty();}}
