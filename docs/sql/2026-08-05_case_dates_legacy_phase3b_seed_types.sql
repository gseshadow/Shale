...
IF OBJECT_ID(N'dbo.CaseDates', N'U') IS NULL THROW 56105, 'Missing dbo.CaseDates.', 1;
...
IF EXISTS(SELECT 1 FROM dbo.CaseDateTypes WHERE SystemKey=N'medical_negligence_discovered') OR EXISTS(SELECT 1 FROM dbo.CaseDates cd JOIN dbo.CaseDateTypes t ON t.Id=cd.CaseDateTypeId WHERE t.SystemKey=N'medical_negligence_discovered') THROW 56106, 'Discarded medical-negligence discovery SystemKey requires separately reviewed reconciliation.', 1;
