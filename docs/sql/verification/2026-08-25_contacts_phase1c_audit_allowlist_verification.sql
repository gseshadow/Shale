/* Contacts Phase 1C read-only effective EntityType allowlist verification.
...
BEGIN TRY
    ...
    IF SESSION_CONTEXT(N'ShaleClientId') IS NOT NULL
        THROW 56200, 'Contacts Phase 1C requires SESSION_CONTEXT ShaleClientId to be NULL.', 1;
    ...
    SELECT N'Phase 1C Contact EntityTypes accepted (expect 6)' CheckName, COUNT_BIG(*) FindingCount
    FROM @Extracted x WHERE x.ConstraintObjectId=@AllowlistObjectId AND x.Value IN('CONTACT_TYPE','SPECIALTY','CREDENTIAL_DEFINITION','CONTACT_CONTACT_TYPE','CONTACT_SPECIALTY','CONTACT_CREDENTIAL');
