/* Contacts Phase 2B read-only effective EntityType allowlist verification.
...
BEGIN TRY
    ...
    IF SESSION_CONTEXT(N'ShaleClientId') IS NOT NULL
        THROW 56200, 'Contacts Phase 2B requires SESSION_CONTEXT ShaleClientId to be NULL.', 1;
    ...
    SELECT N'Phase 2B CONTACT EntityType accepted (expect 1)' CheckName, COUNT_BIG(*) FindingCount
    FROM @Extracted x WHERE x.ConstraintObjectId=@AllowlistObjectId AND x.Value='CONTACT';
