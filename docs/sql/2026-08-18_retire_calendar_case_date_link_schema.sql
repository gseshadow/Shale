...
BEGIN TRY
  ...
  IF @Present=1 AND NOT EXISTS(SELECT 1 FROM sys.columns c JOIN sys.types t ON t.user_type_id=c.user_type_id WHERE c.object_id=OBJECT_ID(N'dbo.CalendarEvents') AND c.column_id=@LinkColumnId AND t.name=N'bigint' AND c.max_length=8 AND c.is_nullable=1 AND c.is_computed=0)
  ...
  IF (SELECT COUNT(*) FROM sys.indexes WHERE object_id=@MappingId AND index_id>0)<>3
   OR NOT EXISTS(SELECT 1 FROM sys.indexes i WHERE i.object_id=@MappingId AND i.name=N'PK_CalendarCaseDateTypeMappings' AND i.is_primary_key=1 AND i.is_unique=1 AND i.is_disabled=0 AND i.has_filter=0 AND (SELECT STRING_AGG(c.name,N',') WITHIN GROUP(ORDER BY ic.key_ordinal) FROM sys.index_columns ic JOIN sys.columns c ON c.object_id=ic.object_id AND c.column_id=ic.column_id WHERE ic.object_id=i.object_id AND ic.index_id=i.index_id AND ic.key_ordinal>0)=N'Id' AND NOT EXISTS(SELECT 1 FROM sys.index_columns ic WHERE ic.object_id=i.object_id AND ic.index_id=i.index_id AND ic.is_included_column=1))
   OR NOT EXISTS(SELECT 1 FROM sys.indexes i WHERE i.object_id=@MappingId AND i.name=N'UX_CalendarCaseDateTypeMappings_EventType' AND i.is_unique=1 AND i.is_disabled=0 AND i.has_filter=1 AND REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(LOWER(i.filter_definition),N'[',N''),N']',N''),N' ',N''),N'(',N''),N')',N'')=N'isactive=1' AND (SELECT STRING_AGG(c.name,N',') WITHIN GROUP(ORDER BY ic.key_ordinal) FROM sys.index_columns ic JOIN sys.columns c ON c.object_id=ic.object_id AND c.column_id=ic.column_id WHERE ic.object_id=i.object_id AND ic.index_id=i.index_id AND ic.key_ordinal>0)=N'ShaleClientId,CalendarEventTypeId' AND NOT EXISTS(SELECT 1 FROM sys.index_columns ic WHERE ic.object_id=i.object_id AND ic.index_id=i.index_id AND ic.is_included_column=1))
   OR NOT EXISTS(SELECT 1 FROM sys.indexes i WHERE i.object_id=@MappingId AND i.name=N'UX_CalendarCaseDateTypeMappings_DateType' AND i.is_unique=1 AND i.is_disabled=0 AND i.has_filter=1 AND REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(LOWER(i.filter_definition),N'[',N''),N']',N''),N' ',N''),N'(',N''),N')',N'')=N'isactive=1' AND (SELECT STRING_AGG(c.name,N',') WITHIN GROUP(ORDER BY ic.key_ordinal) FROM sys.index_columns ic JOIN sys.columns c ON c.object_id=ic.object_id AND c.column_id=ic.column_id WHERE ic.object_id=i.object_id AND ic.index_id=i.index_id AND ic.key_ordinal>0)=N'ShaleClientId,CaseDateTypeId' AND NOT EXISTS(SELECT 1 FROM sys.index_columns ic WHERE ic.object_id=i.object_id AND ic.index_id=i.index_id AND ic.is_included_column=1))
  ...
  IF (SELECT COUNT(*) FROM sys.default_constraints WHERE parent_object_id=@MappingId)<>4
   OR NOT EXISTS(SELECT 1 FROM sys.default_constraints d JOIN sys.columns c ON c.object_id=d.parent_object_id AND c.column_id=d.parent_column_id WHERE d.parent_object_id=@MappingId AND d.name=N'DF_CalendarCaseDateTypeMappings_CDToCal' AND c.name=N'CaseDateToCalendar' AND REPLACE(d.definition,N' ',N'')=N'((0))')
   OR NOT EXISTS(SELECT 1 FROM sys.default_constraints d JOIN sys.columns c ON c.object_id=d.parent_object_id AND c.column_id=d.parent_column_id WHERE d.parent_object_id=@MappingId AND d.name=N'DF_CalendarCaseDateTypeMappings_CalToCD' AND c.name=N'CalendarToCaseDate' AND REPLACE(d.definition,N' ',N'')=N'((0))')
   OR NOT EXISTS(SELECT 1 FROM sys.default_constraints d JOIN sys.columns c ON c.object_id=d.parent_object_id AND c.column_id=d.parent_column_id WHERE d.parent_object_id=@MappingId AND d.name=N'DF_CalendarCaseDateTypeMappings_IsActive' AND c.name=N'IsActive' AND REPLACE(d.definition,N' ',N'')=N'((1))')
   OR NOT EXISTS(SELECT 1 FROM sys.default_constraints d JOIN sys.columns c ON c.object_id=d.parent_object_id AND c.column_id=d.parent_column_id WHERE d.parent_object_id=@MappingId AND d.name=N'DF_CalendarCaseDateTypeMappings_CreatedAt' AND c.name=N'CreatedAt' AND LOWER(REPLACE(d.definition,N' ',N'')) LIKE N'%sysutcdatetime()%')
   OR (SELECT COUNT(*) FROM sys.check_constraints WHERE parent_object_id=@MappingId)<>1
   OR NOT EXISTS(SELECT 1 FROM sys.check_constraints WHERE parent_object_id=@MappingId AND name=N'CK_CalendarCaseDateTypeMappings_Direction' AND is_disabled=0 AND is_not_trusted=0 AND LOWER(REPLACE(REPLACE(REPLACE(definition,N'[',N''),N']',N''),N' ',N'')) IN (N'(casedatetocalendar=(1)orcalendartocasedate=(1))',N'((casedatetocalendar=(1)orcalendartocasedate=(1)))'))
    THROW 57113, 'Mapping defaults/check constraint shape is incomplete or incompatible.', 1;
  DECLARE @TriggerDefinition nvarchar(max)=(SELECT LOWER(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(definition,N'[',N''),N']',N''),N'(',N''),N')',N''),N' ',N''),CHAR(13),N''),CHAR(10),N''),CHAR(9),N'')) FROM sys.sql_modules WHERE object_id=OBJECT_ID(N'dbo.TR_CalendarCaseDateTypeMappings_Tenant'));
  ...
  IF (SELECT COUNT(*) FROM sys.security_predicates WHERE target_object_id=@MappingId)<>4 OR EXISTS(
   ...
   OR EXISTS(SELECT 1 FROM @ExpectedRls e WHERE NOT EXISTS(SELECT 1 FROM sys.security_predicates p WHERE p.object_id=@PolicyId AND p.target_object_id=@MappingId AND p.predicate_type_desc=e.PredicateType AND (p.operation_desc=e.Operation OR p.operation_desc IS NULL AND e.Operation IS NULL)))
  ...
  DECLARE @LinkIndexId int=(SELECT i.index_id FROM sys.indexes i WHERE i.object_id=OBJECT_ID(N'dbo.CalendarEvents') AND i.name=N'UX_CalendarEvents_ActiveCaseDateLink' AND i.is_unique=1 AND i.is_disabled=0 AND i.has_filter=1
  ...
  IF EXISTS(SELECT security_predicate_id,object_id,target_object_id,predicate_type,operation,predicate_definition FROM sys.security_predicates WHERE object_id=@PolicyId
            EXCEPT SELECT security_predicate_id,object_id,target_object_id,predicate_type,operation,definition FROM @UnrelatedPredicates)
   OR EXISTS(SELECT security_predicate_id,object_id,target_object_id,predicate_type,operation,definition FROM @UnrelatedPredicates
             EXCEPT SELECT security_predicate_id,object_id,target_object_id,predicate_type,operation,predicate_definition FROM sys.security_predicates WHERE object_id=@PolicyId)
