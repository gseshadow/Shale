/*
 * Forward-only successor for the durable entity-action audit EntityType allowlist.
 * Organizations Phase 3C; adds structured Organization contact entity types without rewriting audit rows.
...
IF @ExpectedDatabase=N'REPLACE_WITH_APPROVED_DATABASE' OR DB_NAME()<>@ExpectedDatabase THROW 57300,'Set and verify the approved database.',1;
IF @OperatorVerifiedAllTenantVisibility<>1 THROW 57301,'Verify all-tenant visibility before deployment.',1;
IF SESSION_CONTEXT(N'ShaleClientId') IS NOT NULL OR SESSION_CONTEXT(N'PrincipalUserId') IS NOT NULL THROW 57302,'Administrative deployment requires NULL tenant and principal context.',1;
IF USER_NAME() IN(N'shale_app',N'shale_runtime') OR (ISNULL(IS_SRVROLEMEMBER(N'sysadmin'),0)<>1 AND ISNULL(IS_MEMBER(N'db_owner'),0)<>1) THROW 57303,'Use an approved all-tenant administrative principal.',1;
...
BEGIN TRY
    ...
    INSERT @Allowed (Value) VALUES
        ('CASE'), ('CASE_STATUS'), ('CASE_OVERVIEW_CONFIGURATION'), ('LINK_TYPE'), ('CASE_LINK'), ('CASE_LINK_SHARE'), ('CASE_DATE'), ('CASE_DATE_TYPE'),
        ...
        ('ORGANIZATION_TYPE'), ('ORGANIZATION_ORGANIZATION_TYPE'), ('ORGANIZATION'),
        ('ORGANIZATION_PHONE'), ('ORGANIZATION_EMAIL'), ('ORGANIZATION_ADDRESS'), ('ORGANIZATION_WEBSITE');
