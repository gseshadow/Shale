...
DECLARE @BlockerCount bigint=(CASE WHEN @VisibilityConfirmed=1 THEN 0 ELSE 1 END)+@DraftAliasDefinitionCount+@DraftAliasOccurrenceCount+@UnresolvedTypeCount+@DestinationMismatchCount+@OrphanCallerTimeCount+@CrossTenantCount+@ActorMismatchCount;
SELECT 'FINAL_VALIDATION_SUMMARY' SectionName,CONVERT(nvarchar(128),@SessionTenantId) SessionContextShaleClientId,@ParticipatingTenantCount ParticipatingTenantCount,@EligibleSourceRowCount EligibleSourceRowCount,CASE WHEN @VisibilityConfirmed=1 THEN 'OPERATOR_VERIFIED_ALL_TENANT_VISIBILITY' ELSE 'OPERATOR_VERIFICATION_REQUIRED' END VisibilityReadiness,@DraftAliasDefinitionCount DraftAliasDefinitionCount,@DraftAliasOccurrenceCount DraftAliasOccurrenceCount,@UnresolvedTypeCount UnresolvedTypeSourceCount,@DestinationMismatchCount DestinationMismatchCount,@OrphanCallerTimeCount OrphanCallerTimeCount,@FlagSetDateMissingCount FlagSetDateMissingCount,@DatePresentFlagNotSetCount DatePresentFlagNotSetCount,@CrossTenantCount CrossTenantOrBrokenRelationshipCount,@ActorMismatchCount CreatedByTenantMismatchCount,@BlockerCount BlockerCount,CASE WHEN @BlockerCount=0 THEN 'SUCCESS' ELSE 'BLOCKED' END ValidationResult;
