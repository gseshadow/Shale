...
IF EXISTS(SELECT 1 FROM @MappingColumns r LEFT JOIN sys.columns c ON c.object_id=OBJECT_ID(N'dbo.CalendarCaseDateTypeMappings') AND c.name=r.ColumnName LEFT JOIN sys.types t ON t.user_type_id=c.user_type_id WHERE c.column_id IS NULL OR t.name<>r.TypeName OR c.max_length<>r.MaxLength OR c.is_nullable<>r.Nullable OR c.is_identity<>r.IdentityFlag)
...
WHILE @@FETCH_STATUS=0 BEGIN
 IF EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.CalendarCaseDateTypeMappings') AND name=@IndexName) AND NOT EXISTS(SELECT 1 FROM sys.indexes i WHERE i.object_id=OBJECT_ID(N'dbo.CalendarCaseDateTypeMappings') AND i.name=@IndexName AND i.is_unique=1 AND i.has_filter=1 AND i.is_disabled=0 AND REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(LOWER(i.filter_definition),N'[',N''),N']',N''),N' ',N''),N'(',N''),N')',N'')=N'isactive=1' AND (SELECT STRING_AGG(c.name,N',') WITHIN GROUP(ORDER BY ic.key_ordinal) FROM sys.index_columns ic JOIN sys.columns c ON c.object_id=ic.object_id AND c.column_id=ic.column_id WHERE ic.object_id=i.object_id AND ic.index_id=i.index_id AND ic.key_ordinal>0)=@ExpectedKeys)
  THROW 55223,'A named active mapping unique index exists with an incompatible definition.',1;
 IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.CalendarCaseDateTypeMappings') AND name=@IndexName)
 BEGIN
...
IF EXISTS(SELECT 1 FROM sys.security_predicates p WHERE p.object_id=@PolicyObjectId AND p.target_object_id=OBJECT_ID(N'dbo.CalendarCaseDateTypeMappings') AND NOT EXISTS(SELECT 1 FROM @Rls r WHERE r.PredicateType=p.predicate_type_desc AND (r.Operation=p.operation_desc OR (r.Operation IS NULL AND p.operation_desc IS NULL))))
...
WHILE @@FETCH_STATUS=0 BEGIN
 IF NOT EXISTS(SELECT 1 FROM sys.security_predicates p WHERE p.object_id=@PolicyObjectId AND p.target_object_id=OBJECT_ID(N'dbo.CalendarCaseDateTypeMappings') AND p.predicate_type_desc=@PredicateType AND (p.operation_desc=@Operation OR (p.operation_desc IS NULL AND @Operation IS NULL)))
 BEGIN
