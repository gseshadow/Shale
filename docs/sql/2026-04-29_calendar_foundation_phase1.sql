...
BEGIN TRY
    ...
    IF NOT EXISTS (
        ...
        WHERE object_id = OBJECT_ID('dbo.CalendarEventTypes')
          AND name = 'UX_CalendarEventTypes_Global_SystemKey'
    )
    BEGIN
        CREATE UNIQUE NONCLUSTERED INDEX UX_CalendarEventTypes_Global_SystemKey
            ON dbo.CalendarEventTypes (SystemKey)
    ...
    IF NOT EXISTS (
        ...
        FROM sys.indexes
        WHERE object_id = OBJECT_ID('dbo.CalendarEventTypes')
          AND name = 'UX_CalendarEventTypes_Tenant_SystemKey'
    )
    BEGIN
        CREATE UNIQUE NONCLUSTERED INDEX UX_CalendarEventTypes_Tenant_SystemKey
            ON dbo.CalendarEventTypes (ShaleClientId, SystemKey)
            WHERE ShaleClientId IS NOT NULL
              AND SystemKey IS NOT NULL;
    END;
    ...
    FROM (VALUES
        ...
        ('DISCOVERY_DEADLINE', 'Discovery Deadline', 35),
