/* Phase 2B forward-only audit vocabulary. Apply manually after Phase 2A and before application code. */
SET XACT_ABORT ON;
BEGIN TRY
 BEGIN TRANSACTION;
 ...
 IF CHARINDEX(N'''FIELD_CONFIRMATION_POLICY''',@entity)=0 OR CHARINDEX(N'''SAVED_VALUE_CONFIRMATION''',@entity)=0 BEGIN
  DECLARE @entityReplacement nvarchar(max)=N'('+@entity;
  IF CHARINDEX(N'''FIELD_CONFIRMATION_POLICY''',@entity)=0 SET @entityReplacement+=N' OR [EntityType]=''FIELD_CONFIRMATION_POLICY''';
