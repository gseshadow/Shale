...
class CompatibilityCaseDateEditorTest {
 @Test void createsExactlyNineConcurrencyAwareIntents() {
  ...
  for(var k:MigratedCaseDateKey.values()){before.put(k,new CompatibilityCaseDateState(k,k.systemKey(),null,null,true,null,null,null,new CompatibilityCaseDateMutation.ExpectedAbsent(caseRv)));after.put(k,null);}
  before.put(MigratedCaseDateKey.DATE_OF_INJURY,new CompatibilityCaseDateState(MigratedCaseDateKey.DATE_OF_INJURY,"date_of_injury",old,null,true,10L,101,dateRv,null));
  before.put(MigratedCaseDateKey.STATUTE_OF_LIMITATIONS,new CompatibilityCaseDateState(MigratedCaseDateKey.STATUTE_OF_LIMITATIONS,"statute_of_limitations",old,null,true,11L,102,dateRv,null));
  before.put(MigratedCaseDateKey.TORT_NOTICE_DEADLINE,new CompatibilityCaseDateState(MigratedCaseDateKey.TORT_NOTICE_DEADLINE,"tort_notice_deadline",old,null,true,12L,103,dateRv,null));
