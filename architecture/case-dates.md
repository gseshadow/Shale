...
The preflight and post-validation are deliberately all-tenant operations. In Azure Data Studio or SSMS, use the approved deployment/migration database connection—not the tenant-scoped `shale_runtime` application login—open a fresh query against the deployed Shale database, and leave `SESSION_CONTEXT(N'ShaleClientId')` unset (`NULL`). The principal must be a verified `db_owner` member or `sysadmin` and must retain RLS-authorized visibility to all tenants; do not set or clear a read-only tenant context inside the package. Return every result grid, including `PREFLIGHT_VALIDATION_SUMMARY`, for review. The summary reports the session context, visible participating tenant count, and eligible source-row count. A non-null tenant context, a non-administrative principal, zero visible participating tenants, or zero visible eligible rows produces `VISIBILITY_NOT_CONFIRMED` and blocks both seed and backfill review. This is an enforceable minimum, not permission to infer readiness from an empty result set.
