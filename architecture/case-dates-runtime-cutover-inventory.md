...
This gate converts only the desktop Cases surface rooted at `CasesController` and its two existing presentations: the JavaFX `TableView` grid and the `FlowPane` card board produced by the shared `CaseCardFactory`. Production composition is `SceneManager.createCasesView` → `CasesController` → `CaseDao.findCasesViewPage` for the bounded base result and `CaseServicePort.projectMigratedCaseDates` for one set-oriented enrichment call per loaded page. Row/card selection, double-click/Enter navigation, the shared card layout, status/search filters, infinite card paging, full grid loading, load generations, and base ordering remain unchanged.

The grid and board display four of the nine fixed meanings: timed `intake` (rendered with the existing date-only list presentation), and all-day `date_of_injury`, `statute_of_limitations`, and `tort_notice_deadline`. Every displayed value is merged by `CaseId` from `MigratedCaseDateProjectionDto`; an omitted projection cannot disclose a cross-tenant case, and a visible case with no occurrence receives the DTO's coherent empty nine-slot state. The controller never reads those values from `CaseRow`. Arbitrary custom types do not occupy fixed slots, flags do not fabricate occurrences, and projection duplicate detection remains fail-fast.

Intake and statute sorting participate in SQL paging, so Cases-view `findPageInternal` uses an authoritative `CaseDates`/stored `CaseDateTypes.SystemKey` apply for those order expressions before `OFFSET/FETCH`; it also supplies authoritative aliases for the bounded base row. Search and status filtering are not date filters. Display enrichment never reorders a page. The MyShale invocation of the shared internal query explicitly retains its legacy compatibility mode and is not classified as converted. Export walks the entire globally ordered Cases result in existing 500-row pages, then calls the shared projection once for the collected IDs; the projection itself performs one occurrence query per 500 unique valid IDs. There is no per-case type or occurrence query.

The Cases export boundary is `CasesController.exportCases` → `CaseExportService.exportCases` → `CaseDao.listCasesViewForExport` plus the shared projection → consumer-specific `ExportCaseRow` → existing CSV mapping or `CaseXlsxExporter`. Its eleven headers and order, UTF-8 CSV handling, typed date cells, Unicode-safe Excel truncation, background execution, disabled-menu progress state, PHI read audit, and PHI-safe user error remain intact. Reports continue using their existing report DTO/query and are deliberately deferred.
