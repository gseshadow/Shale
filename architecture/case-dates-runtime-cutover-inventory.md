...
This converted inline path neither reads the nine legacy DTO date values nor calls a legacy compatibility writer and performs no dual-write. Deferred compatibility remains narrowly in `CaseController` new-case intake and its broad multi-field Details/Overview saves, `CaseServiceAdapter.updateCaseCoreDetails`, `ApiReadController`, `shale-web/src/api.ts`, `shale-web/src/App.tsx`, and the documented projections in `CaseDao`, `OrganizationDao`, `ContactDao`, and `CalendarFeedDao`. Migration/validation/rollback SQL and their exact contract tests remain the non-runtime allowlist. The next gate is separating the remaining broad desktop non-date save from its legacy date-shaped DAO signature, followed by intake/API/web atomic create conversion; it must not be partially converted.
